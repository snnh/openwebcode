...
- **纯图片/附件消息可直接发送**：只贴图或只挂 PDF/文件（不写文字）时发送按钮可用。正文由服务端落占位文本（`[Image]` / `[Image ×2]` / `[File: 名称]`），**会话文件格式（`messages.jsonl`）不变**；历史里能看出这条「只有图/附件」，会话标题取该占位而不是空白。顺带修掉两处相关缺陷：① 空正文块不再下发给 provider（部分端点会以 400 拒收空 text part，历史遗留的空块也会被跳过）；② 会话标题派生跳过 `[Attachment …]` 附件引用块，此前「附件 + 正文」消息的标题会取附件正文的前 80 字符。REST 校验收紧为「正文非空，或必须带 images/attachments」，两者皆空仍返回 400。
- **运行中可直接更换模型与思考档**：输入区的模型弹层此前在运行中被禁用（服务端对 `PUT /api/sessions/:id/config` 的模型类字段一律 409）。现在模型（含跨服务商）与思考开关/强度都可在运行中随时切换：不打断在途请求，主循环下一轮现读新配置即生效，提示条写明「下一轮生效」；若此刻正跑在限流降级的备选模型上，切换会放弃该降级、直接回到新选的主模型。会话的 agent 模式、沙盒、环境等仍须空闲时改（仍返回 409）。

### 新功能

- **会话组织：手工分组、归档、多选批量**：会话列表可按手工标签组整理（会话项上的文件夹按钮移动/移出/新建分组，组头可折叠、重命名、删除——删组只把会话送回「未分组」），分组与归档状态存服务端会话 meta（换浏览器一致，`PATCH /api/sessions/:id` 新增 `group`/`archived`）。**归档**把会话移出默认列表、进底部默认收起的「已归档」区，并顺带释放它占用的常驻资源（该 cwd 的索引内存与文件监听、repo map 扫描缓存；索引文件仍在磁盘，下次访问自动重建），不删历史/快照/artifacts，发送消息即自动撤销归档；出于安全考虑**有活动就拒绝归档**（运行中的 run、仍在跑的后台任务、开着的终端都返回「先停掉再归档」）。列表顶部「多选」按钮支持批量删除（二次确认列出标题）、归档、置顶；批量归档自动跳过仍在运行的会话并提示。搜索现在也能命中分组名。

### 修复与优化

- **常驻资源会自己收敛（前端内存 + 服务端索引/监听）**：① 前端「向上翻页加载的更早消息」此前按会话**永久保留**（只在 resync/删除会话时清），会话开得多且各自翻过历史后标签页越用越重——现在单会话最多保留 300 条（约 3 页，超出丢最旧并把 `hasMore` 复位以便再翻），并且只保留最近用过的 10 个会话的分页缓存（LRU，超出的整体释放）。② 服务端的符号索引工作区（内存索引 + 文件监听）此前按 cwd 常驻且**没有淘汰**，打开过的项目越多占得越多——现在空闲 30 分钟自动释放、同时最多保留 8 个（每 5 分钟清扫一次；索引文件留在磁盘，下次访问自动重建，索引内容与语义不变），归档会话即时释放；顺带修掉 repo map 扫描缓存按会话累积的泄漏（会话删除时未清理）。
- **待回答卡不再被截断、可收起（ask_user / 权限确认 / 计划批准 / 运行队列）**：这四类「等你操作」的方块渲染在消息列表之外、输入框之上，此前既没有高度上限也不能收起——主列（`.wb-main`）本身 `overflow: hidden` 且不滚动，所以卡片一高（多选项、长计划、带参数预览的权限卡），底部（选项/`提交回答`）连同**输入框一起被裁掉，且没有任何方式滚到它**。现在四类共用同一个「待回答区」：卡片内容区限高（可用高度一半 / 60vh / 扣掉列表保底与顶栏输入框，三者取小）+ 卡内滚动；消息列表保底约 120px（约 3 行上下文）；输入框不再被挤走。同时加入**手风琴 + 收起记忆**：同时只展开队列最早的一张（权限确认在前，因为它卡住本轮运行），其余收成一行摘要（`待回答` 标记 + 主操作按钮，已选项可直接提交）；点卡头三角收起/展开，收起状态按卡片记在 sessionStorage（本会话切标签、刷新都不丢），新出现的卡默认展开；新卡出现时消息列表自动滚到底。权限卡因此改由 ChatView 的待回答区渲染（此前在消息流里），它的位置从「随对话滚动」变成「固定在输入框上方」，不会再被新消息顶走。
- **subagent 显示：一眼分清是哪一个、看得到模型、完成后好回顾**：主区子代理标签改名为 `#序号 · 代理名 · 任务摘要`（序号＝本会话内该次派生的发生顺序，同名 `explore` 靠序号 + 摘要区分；swarm 为 `#n Swarm ×N 项`），固定按发生顺序排列，当前标签会自动滚入视野，行尾新增「全部 ⋯」下拉列全并可直接跳转；**没有子代理时整条标签栏不再渲染**（此前常驻 34px；有任务清单 chip 时保留该行），子代理完成后标签留在原位显示完成/失败点待手动关闭。子代理标签页里每个运行行与组头都标出**实际生效的模型**（此前只有展开卡片里能看到，这是「看不出选了什么模型」的原因——数据本来就在 `subagent.started` 里），角色徽标也补到运行行上；已完成条目默认显示**结论摘要 + 「复制结论」**，回顾产出不必逐项展开转录；消息轨道的子代理卡片展开后新增「在标签中打开」入口；侧栏子代理面板新增状态筛选（全部/运行中/完成/失败）。
- **子代理列表按批次收敛，`/clear` 一并清空**：模型开始新一批派生（新一轮 run 的首次 `subagent`/`spawn_swarm`）时，上一批**已结束**的子代理从标签栏与面板移除（仍在运行的不动，避免并行场景丢掉监控入口）；`/clear` 清空上下文时该会话的子代理运行、标签与面板条目整体清空（消息推导的历史条目按 taskId/toolCallId 一起过滤）。两种情况下对话消息里的工具卡都保留，随时仍可回看结果。
- **首屏不再预下载 Monaco（省约 4 MB）**：`vite.config.ts` 的 `manualChunks` 用对象形式时，Vite 的 `__vitePreload` helper（所有懒加载 chunk 的静态依赖）会被并进 monaco chunk，导致入口与 MarkdownImpl/各面板都静态引用它，`index.html` 里因此出现 4 MB 的 `modulepreload`——页面一打开就下一遍全量编辑器。现在改为函数形式：preload helper 单独成 chunk（约 1 KB），Monaco 及其语言模式保持按需懒加载。首屏因此少下约 4.0 MB JS（gzip 约 1.0 MB）+ 162 KB Monaco CSS（gzip 25 KB）。新增构建守卫：`index.html` 里 modulepreload 的 JS 必须落在入口预算内，这类回归会被 `npm run build` 直接挡下。
- **代码高亮成本护栏 + 编辑器预热**：Shiki 在渲染线程同步执行（实测约 5ms/KB，2000 行 ≈ 265ms），此前打开大文件预览或大工具卡会卡住界面。现在超过 60k 字符的内容直接渲染纯文本并提示（仍可复制/在编辑器中打开），高亮结果在 `startTransition` 中提交（先显示可交互的纯文本再替换）；悬停/聚焦「在编辑器中打开」会提前预热 Monaco chunk。顺带把侧栏文件预览的查询键与编辑器/浮层统一（`file-content`），预览过再打开编辑器不再重复取数；写事件标脏也覆盖已打开的预览/编辑器内容。
- **文件树加载变快、不再静默丢条目**：侧栏「文件」视图的目录查询与文件预览此前 `staleTime=0`，每次展开/折叠、切回视图都会重新请求一次 `fs.list`（虽然缓存内容仍即时可见）。现在目录/预览读查询带 10s 客户端缓存，排序结果按目录条目记忆化（父组件因选中文件重渲染时不再重复 O(n log n) 排序）。同时修掉一个静默缺陷：core 单目录最多返回 256 个条目，超出部分此前既不显示也无提示（用户会以为文件不存在）——现在列表末尾给出「条目过多，仅列出前一部分」并指引用 Quick Open 或 `@` 搜索（行为已在 `docs/protocol.md` 注明）。写类事件（工具写文件、编辑器保存、run 结束）会把 SCM 与文件树一起标脏，并在静默 400ms 后**合并重取一次**：连续写文件不再让 git status 与目录请求成倍放大，界面上仍是自动刷新。
- **源代码管理面板加载变快**：每次打开/切回 SCM 标签、切会话、每次文件写入事件都会重跑 `git status`（大仓库一次全树扫描，且此前每次读取都多跑一次 `rev-parse` 探针、diff 三次）。现在服务端读路径加了 1.5s TTL 缓存与并发去重（同一时刻的重复请求共享一次 git 调用），仓库探针改为 10s 缓存、`status` 直接按 `git status` 的失败信息判定「不是仓库」而省掉一次进程；任何写入（工具写文件、编辑器保存、提交/暂存、run 结束）都通过 `scm.updated` 事件立即失效缓存，不会看到旧状态。前端给 SCM 读查询加了 10s `staleTime`（此前全局 staleTime=0，切回标签必重拉），run 终态补一次失效以覆盖 `bash` 等不经 `scm.updated` 的工作树改动。
- **会话顶栏操作行的分隔竖线不再被按钮边框框住**：`|` 分隔此前是按钮自身的行内内容，落在「中断」/「手动快照」的边框之内；现在改在按钮框外绘制（不占布局宽度，跟随按钮折行），桌面与手机端间距与其它分隔保持一致。

### 性能与资源

- **静态资源与 API 响应传输量下降一个量级**：WebUI 的文本资产（JS/CSS/HTML/SVG）在构建期产出 Brotli 同伴文件（`dist` 131 个文件 16.7 MB → 3.5 MB，省 79%），服务端优先直发同伴、客户端不支持或同伴缺失时自动回落原文件，运行期零压缩 CPU；带内容散列的资源改 `immutable` 强缓存（此前每次访问都要 304 重验，129 个 chunk 一轮刷新就是 129 次往返），入口与清单文件保持每次重验以免拿到已删除的 chunk。API 的 JSON 响应启用 br/gzip（阈值 1 KB）——会话详情这类含长消息正文的响应实测 8 KB+ → 330 B。
- **多会话长跑内存收敛（服务端）**：会话消息整表缓存此前只限会话条数（32 个）不限字节，工具结果多的大会话单条缓存可达数百 MB，多会话来回切换会持续堆叠——现在按 `messages.jsonl` 字节计权重，单会话超过 32 MB 不驻留、累计超过 256 MB 按 LRU 逐出最旧（内存对象通常为文件字节的 2-3 倍，估算零额外 CPU）；会话删除时释放事件总线里的按会话序列记账（此前随「历史上创建过的会话数」单调增长）；后台任务的未读提示按会话设上限（未读取时不再无限攒字符串）。
- **测试套件分层精简重写（工程侧）**：按「冻结契约 + 分层精炼」重写测试，去掉样式/类名与 mock 调用计数断言、整份对象逐字比对、跨文件重复覆盖，同构用例合并为表驱动：总行数 27.2k → 16.7k、用例 948 → 441（core 的协议 fixture、路径策略与沙盒 deny 矩阵全冻）。RPC 帧语义、provider 请求体形状、会话迁移、快照回滚、账本与安全门禁等契约锚点逐项核查保留。

### 安全

- **图源抓取的 SSRF 网关补齐 DNS 复查**：chat 模式的 vision 图源（URL 由模型给出）此前只校验 IP 字面量，`域名解析到 127.0.0.1/内网` 的图源可绕过——现在与其它抓取通道一致，逐跳（含起始）做 DNS 解析复查，且校验先于请求发出。
...
- **Agent 集群协作升级（spawn_swarm）**：成员互相可见——roster 系统贴广播全员角色/模型/分工摘要，成员起止写生命周期帖；讨论板支持结构化交流——帖子带短 id 与协作 kind（finding/question/request/blocker/progress/decision）、私聊（`to=`，仅收发双方可见）、引用回复（`replyTo`）与优先级；新增 `swarm_wait` 工具——成员可一次性等待他人回复（默认等发给自己的私聊/@提及，`from=` 等指定成员且其已结束时提前返回，上限 300 秒，等待期间占用并发槽）；新增 `synthesize` 参数（默认关）——全部成员结束后以指定档位模型做一次无工具合成轮，输出结构化报告（关键发现/分歧/风险/决议/待办），失败自动回落原始结论；Board digest 升级为 kind 分组统计 + 决议/阻塞置顶；WebUI swarm 卡片逐项显示角色徽标与模型名，并实时展示合成轮「汇总」行。

## [1.10.6]

### 安全

- **Chat 模式鉴权对齐**：移除 chat 对话路由的局域网免认证通道，与 code 模式统一走访问令牌鉴权，消除局域网内未认证调用模型与工具的通道。
- **沙盒凭据边界收紧**：`~/.ssh` 移出沙盒默认只读挂载，改为会话级显式开关（新建会话对话框/沙盒面板可开）；core 进程环境白名单显式剔除 `OWC_ACCESS_TOKEN`，环境变量注入的访问令牌不再泄漏给沙盒内命令。
- **只读命令自动放行白名单收敛**：移除可被滥用于读取任意文件/执行写操作的条目，未知命令回到人工确认。
- **denyPaths 强化**：POSIX 侧在路径解析后复核 deny 命中（防硬链接/绑定挂载绕过文本前缀匹配）；Windows 侧词法层拒绝 NTFS 备用数据流（`file:stream`）路径。
- **WSB 回连加固**：回连握手改用一次性令牌，监听地址按配置 hostIp 绑定。
- **扩展宿主环境白名单**：扩展子进程只继承最小环境集；`/api/exec` 不再接受 network 覆盖参数。
- **core 健壮性**：手写 JSON 解析器增加节点数量与重复键预算，抵御复杂度型拒绝服务；长任务回复缓冲超限截断。

### 修复与优化

- 修复 Markdown 围栏代码块语言标识未透传导致的 shiki 高亮失效；外部图片引用增加降泄漏处理。
- Web 草稿持久化改存 sessionStorage（刷新不丢、关闭标签页即清除），旧 localStorage 残留草稿自动清理。
- 上下文构建：驱逐条目改片段级缓存失效，长会话每轮不再全量重建视图（配套基准纳入发布门槛）。
- 索引监视事件改定向增量刷新，消除去抖窗口内的全盘重扫。
- 流式订阅下沉到消息卡片级，消除流式期间顶层逐帧连带重渲染。
- `/api/sessions/:id/context` 响应不再携带全量消息体；bash 工具输出累积增加字节上限；RPC 帧解码改分块累积，消除大帧 O(n²) 拷贝。

## [1.10.5]

### 修复与优化

- 内置工具参数异名容错：模型误用其他产品的参数名（如 `bash` 的 `command`、文件工具的 `file_path`、`edit_file` 的 `old_string`/`new_string`/`replace_all`、后台任务的 `task_id`）时自动归一为规范参数名再执行，权限判定与确认卡片同步生效；参数仍缺失或非法时报错改为可诊断信息（点名缺失参数、期望形态与实收键），未知工具名单独报错，不再笼统返回 "Unsupported or invalid tool call"。
...
- **子代理默认走模型审核（review）**：主循环派发的 `subagent` / `spawn_swarm` 成员，其内部工具调用（写文件、bash 等）不再逐个弹人工确认，先由审核模型判断——判 LOW 直接放行（记 `permission.reviewed` 审计事件）；快速审核模型未配置、判 HIGH 或审核失败仍转人工；`git_commit` 恒人工。主 agent 权限模式切到 `yolo` 时子代理同步 `yolo`（只跳确认、不扩沙盒）。手动启动的子代理仍随会话权限档（该路径后续计划归档）。
- **同一消息的多个 subagent 调用可并行执行**：原来逐个串行；「子代理并行数」（设置 → 上下文与运行，`OWC_SUB_AGENT_CONCURRENCY`，1–16，默认 2，热生效）决定同一条消息全为 subagent 调用时的同时运行数量（1 = 逐条串行），结果仍按原调用顺序落盘；只限并发数，调用一个不少。spawn_swarm 成员并发改为独立设置「spawn_swarm 并发成员数」（`OWC_SPAWN_SWARM_CONCURRENCY`，2–16，默认 4，热生效），默认值不变。

### 新增

- **快速模型超时设置项 `fastModelTimeoutMs`**（设置 → 模型选择 →「快速模型超时（毫秒）」，环境变量 `OWC_FAST_MODEL_TIMEOUT_MS`，范围 1000–900000，默认 60000，**热生效**）：快速模型的单次尝试超时不再固定 60 秒，慢速思考型快速模型可调大（调大后压缩/内容透镜的失败判定相应放宽；超时仍按失败重试，最多 2 次尝试）。作用于上下文压缩、内容透镜、fast 角色子代理等所有快速模型通道。
...
- **新增 `read_media` 工具**：支持视觉输入的模型可读取工作区本地或公共 URL 的图片与视频（图片 png/jpeg/webp/gif；视频 mp4/quicktime/webm/avi/flv/mp2t/mpeg/3gp 等，仅 OpenAI 兼容 provider 且模型声明 `video` 输入模态时可用）。媒体类型按文件魔数识别（扩展名仅作裸流视频兜底），URL 走与 web_fetch 相同的 SSRF 防护链；图片约 5 MB / 视频 20 MiB 上限，超限报错并指引压缩后重试。工具仅在当前模型声明图片/视频输入模态时注入，描述随能力生成。视频支持模型元数据：kimi-k2.6/k2.7/k3、qwen3-vl/qwen-vl-*、ernie-4.5-vl/ernie-5.0 系列（其余可在模型目录手动声明）。

### 修复

- **二次压缩不再丢失第一轮摘要**：同会话的后续压缩会携带既有摘要求模型合并保留要点（规则降级路径同样拼接），视图不再因新压缩记录覆盖而丢失前次要点。
- **运行中强制压缩补当前轮保护**：压缩区段不包含本轮触发用户消息，边界对齐不拆分 tool_call/tool_result 配对批次（消除 provider 配对修复静默丢弃的工具结果）。
- **压缩水位口径修正**：「工作预算」改为上下文窗口扣除系统侧占用（系统提示/工具 schema/repo map）与输出预留（窗口 1/8），小窗口模型不再触发过晚直接 400。
- **Provider 上下文溢出一次性安全恢复**：捕获明确的 context-length 类 400 时自动强制压缩一次并重试（事件带 `reason:"overflow_recovery"`），仍溢出则以可行动错误收尾。
- **水位 100 = 关闭阈值型强制压缩**（建议水位仍为 −15 个百分点）；溢出恢复不受此开关影响。
- **压缩记录锚定消息 id**：按活动路径消息 id 定位压缩边界（旧记录回退索引）；分叉/编辑到边界以下时不再误隐藏未总结的新分支内容。
- **「正在压缩上下文」转圈不消失自愈**：run 终态、会话刷新、服务端无活跃 run 对账时清除残留的运行中压缩占位。
...
- **bubblewrap 下 deny 路径不存在时不再向命令 stderr 刷警告**：默认 deny 路径（如 `.env`）通常不存在，此前每条命令都会混入 `bwrap deny path skipped` 输出，现已消除。
- **POSIX 命令子进程 stdin 改为 /dev/null**：修复沙盒命令抢读 RPC 管道导致会话间数据串读 / 主循环异常退出的问题。

## [1.9.11] - 2026-08-30

### 新增

- **性能面板「内存占用」统计**：监控开关开启时展示 node 主进程（RSS / 堆）、core 进程与扩展宿主的实时内存占用；来源不可用时显示「—」（旧版 core 二进制自动降级，不影响其它指标）。数据经 `/api/metrics` 按需拉取，面板关闭时零开销，默认每 5 秒刷新。
- **core 新增 `core.stats` RPC**：返回当前进程内存（Linux `VmRSS` / Windows `WorkingSetSize`），作为性能面板与诊断的内存数据源；server 侧按方法存在性降级，可与旧版执行器混跑。

### 修复

- **`core.stats` 查询超时保护**：内存统计查询显式 5s 超时，core 未启动/僵死时 `/api/metrics` 快速降级为「不可用」，不再被默认 130s 超时拖挂（前端 5s 轮询不堆积请求）。
...
- **系统提示词与工具描述全面精简**：
  - 工具目录改为每工具一句话摘要（超长截断，完整规则与参数以工具定义为准），每轮系统提示词净减约 600-900 token。
  - plan 模式两个分支明确「写/执行工具仍列出但会被拒」，减少只读阶段的无效写工具调用。
  - goal 模式段英语化，并融合 KimiCode /goal 自主推进语义（内部计划、子步骤间不暂停确认、不重问可从代码库回答的问题、不可恢复阻塞才停下报告）。
  - 子代理任务引导自包含化：subagent/spawn_swarm 工具描述与子代理目录段明确「子代理看不到当前对话、项目约定文件与会话状态」，需读取的文件须在任务提示词中显式列出；explore/general 子代理系统提示词收紧。
  - remember 描述改为「仅用户明确要求记住时才使用」，不再预设默认 scope（project/global 按事实性质选择）。
  - bash 描述删除 shell 例句与「sandbox-native」断言，新增「沙盒开启时部分命令可能不可用，优先使用工作区内路径」提示。
  - ask_user/read_file/edit_file/glob/grep/test_runner/task_output 描述精简去重；Pi 基线、Work discipline、Communication、swarm 段措辞收紧。
...
- **Composer 控制条布局重构**：
  - 窄屏（≤768px）控制条单行化：附件/权限模式/对话模式/模型选择器与发送按钮同行，不再折成两行；空间不足时只有模型名收缩省略，其余按钮保持刚性，互不重叠（修复此前菜单盒子可被压至小于内容、nowrap 文字溢出盖住右侧按钮的重叠问题）。
  - 权限/模式/模型三个菜单按钮移除箭头图标，省出的空间用于运行中行为选择——由 popover 弹层改为「补充｜续跑」分段单选控件常驻直接点选（默认「补充」，即插入正在运行的回合）。
  - 模型按钮只显模型名（去掉 provider 前缀，完整模型名在底部状态栏展示）；思考程度徽标（如「· max」）始终完整显示，收缩截断只作用于模型名。
  - 运行中「发送将进入 Steering 队列」提示文本与脉动圆点移除——运行状态统一由会话头指示。
- **`/clear` 同步清空任务清单**：清空上下文后旧任务对模型与 UI 均已失效，内存态任务清单一并清空并广播空清单事件，标签栏 chip 同步消失；后续 `todo_write` 从空开始。
