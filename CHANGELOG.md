...
- 会话格式升级扩展升级至 `0.1.3`，新增孤儿工具调用清理步骤，减少旧会话在 Chat/Responses 严格端点上的回放 400。

## [1.10.7]

### 修复

- 深度优化 Chat/Responses 工具调用回放：保留 Responses 原始 `fc_*` item id，稳定处理 Chat 工具输出 ID 不一致和孤儿调用，减少兼容网关 400 错误。

## [1.10.6]

### 修复

- 修复 Chat 工具调用与工具输出标识不一致时产生 `No tool output found` 400 错误。
- 保持标准精确 ID 配对，并对历史消息提供顺序兜底配对。

## [1.10.5]

### 修复

- 修复 OpenAI Responses 与 Chat 兼容协议中孤儿工具调用回放导致的 400 错误。
- 更新会话格式升级扩展，清理无对应工具输出的历史调用。
...
- 优化事件总线生命周期：服务关闭和测试 teardown 时释放合批定时器、历史缓存与监听器，降低长期运行占用。
