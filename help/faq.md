...
把 dsh（DeepSeek Harness）的官方 Web 界面与插件生态托管到 openwebcode 里：设置 → **通用** 打开「启用 dsh 兼容模式」，左侧活动栏底部出现 `layers` 图标，点击在新标签打开 dsh UI（独立端口，默认 3211），会话/模型/权限模式与主工作台共用。它默认关闭、属于实验功能，且 dsh 插件是**可信代码**（安装即信任，不经沙盒）——完整的支持清单、边界与排障见 [dsh-compat.md](./dsh-compat.md)。

### Q: 怎么装 dsh 插件？

把插件包目录放进 `<数据目录>/dsh-plugins/`（`npm pack` 解开即可，插件自己的第三方依赖装在各插件目录里），启用态与配置写在 `<数据目录>/dsh.json`（缺省全部启用），然后重启服务或把 dsh 兼容模式开关关一下再开。翻译层只提供 cordis / schemastery / dsh-tools 三个垫片包，依赖其它 `@deepseek-ai/*` 运行期包的插件会如实报 `incompatible`。详见 [dsh-compat.md](./dsh-compat.md#安装-dsh-插件)。
...
3. **自动压缩水位强制压缩**（默认 85%，可在设置「上下文」页签调整）：快速模型做结构化概览（输出经格式校验防复述原文，不合格自动纠偏重试；快速模型彻底失败时自动降级为规则截断，强制压缩不中断）。压缩调用快速模型的输出上限在同页签「压缩输出上限」调整（默认 65536 tokens，思考型模型需要更大余量）
...
用于压缩、标题生成、翻译等旁路任务的低延迟模型，从已启用服务商里选，复用该服务商的接口和密钥。不是必须：不配时压缩走纯规则截断，概览压缩不可用并提示。选个便宜快的模型体验更好。思考型快速模型的推理会消耗输出预算：正文为空时自动翻倍预算重试一次，仍为空则回退使用推理通道内容，「快速模型返回为空」的概率大幅降低。

### Q: 怎么调整上下文驱逐与压缩？

底部「上下文」面板可热切换 `lag`（滚动）/ `interval`（定期批量）/ `off`（仅手动），改保留轮数和回写预算，逐条驱逐/恢复/pin，看 artifact 原文——这些驱逐相关段落由 context-saver 扩展提供，扩展停用时不显示。压缩是核心功能：自动压缩水位（默认 85%）与压缩输出上限（默认 65536 tokens）在设置「上下文」页签调整，手动压缩按钮始终在面板「压缩」区。会话运行中这些写操作暂时禁用，避免与 agent 构建上下文竞态。
...
能。手机（≤768px）为单列移动布局：导航收进左侧滑出菜单、底部面板变全屏 sheet，下发任务、看状态、处理权限卡、切换会话都可用；平板竖屏（768–1024px）直接用桌面三栏布局。浏览器「安装到主屏」后有 PWA 壳，不做离线缓存。手机访问意味着非回环监听，令牌配置见上面的问答。详见 [usage.md 的移动端一节](./usage.md#移动端与-pwa)。
