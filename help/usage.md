...
- **界面**：左侧可折叠会话栏（新建 / 重命名 / 删除 / 分享 / 分支），中间窄列消息流，底部输入框。支持 Markdown / LaTeX 渲染，粘贴或选择图片上传（小图直接内嵌，大图落盘为引用，单条消息最多 3 张）。
- **会话树**：每条 assistant 消息可「重新生成」（从该点长出新分支，旧分支不丢）；会话可分支 / fork / 回溯到任意消息继续。
