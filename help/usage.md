...
完整键位见设置 → **快捷键** 页签，暂不支持自定义键位。输入框聚焦时全局快捷键不抢键。设置本身是一个整页对话框：左侧导航轨按「个人偏好 / AI 与服务 / 能力与连接 / 系统」分组列出页签（外观、通用、会话默认、快捷键、模型目录、模型选择、上下文、联网服务、模型定价、提示词、技能、扩展、远程访问、通知、服务信息），列表带搜索框，按分区/分组/字段名（中英文）直接定位；界面各处的设置入口（如「前往模型目录」「打开模型设置」）会深链到对应页签。
