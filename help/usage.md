...
显式覆盖：`OWC_ACCESS_TOKEN`（≥32 字符）固定令牌；`OWC_ALLOWED_ORIGINS`（逗号分隔的 http(s) 源）限定浏览器来源，缺省放行与访问地址同源的请求；也可在设置 → 远程访问 →「允许来源列表」直接保存（重启生效）。非回环 + 令牌只是准入门槛，仍建议只在受信网络使用。
...
完整键位见设置 → **快捷键** 页签，支持自定义：点击任一键位进入录制（按新组合键确定、`Esc` 取消），与其它命令冲突的组合键会被拒绝并提示；每行可「恢复默认」，页脚「重置全部」。自定义键位保存在本机浏览器（localStorage），不同设备互不影响；显示与生效始终一致（命令面板与快捷键表展示实际键位）。输入框聚焦时全局快捷键不抢键。设置本身是一个整页对话框：左侧导航轨按「个人偏好 / AI 与服务 / 能力与连接 / 系统」分组列出页签（外观、通用、会话默认、快捷键、模型目录、模型选择、上下文、联网服务、模型定价、提示词、技能、扩展、远程访问、通知、服务信息），列表带搜索框，按分区/分组/字段名（中英文）直接定位；界面各处的设置入口（如「前往模型目录」「打开模型设置」）会深链到对应页签。
...
**设置 → 会话默认** 可以预设新建会话的取值：默认模型与默认权限模式存在本机浏览器（不预设则按服务商列表与 `ask` 兜底）；server 侧默认——默认思考力度 / 默认快照方式 / 快照后端——也在同一页签渲染（原在「通用」页签），保存进 `server-settings.json`。
...
- **纯图片/附件消息**（不写文字也能发）：只贴图或只挂 PDF/文件时发送按钮可用，正文由服务端落占位文本（`[Image]` / `[Image ×2]` / `[File: 名称]`），会话文件格式不变，历史里能看出这条「只有图/附件」，会话标题也取该占位而不是空白
...
- **压缩检查点行**：`/compact` 或达到自动压缩水位（默认 85%）强制压缩开始时，消息流尾部出现「正在压缩上下文」活动行；完成后原位沉降为常驻检查点行（模式徽标 + 手动/强制标注 + 被压缩条数与 token 估算），带摘要的记录可展开查看摘要与指令清单；压缩失败转为常驻错误行（可关闭）。快速模型输出经格式校验防复述原文（不合格自动带原因纠偏重试一次）；强制压缩遇快速模型彻底失败时自动降级为规则截断照常完成。压缩输出上限可在 **设置 → 上下文** 调整（`compactMaxTokens`，默认 65536，热生效；思考型模型推理消耗输出预算，需要更大余量）。多次压缩各自留行，刷新后从上下文账本逐条还原；行只是视图投影，历史消息原文不动
...
- **权限模式（四档）**：输入区权限弹层切换，随会话保存；运行中也可随时切换——切换后挂起的权限卡片按新档自动结算（新档下无需审批的直接放行继续执行；本机会话的 HOME 外路径审批除外，任何档都要人工）：
...
- **子代理权限与并行**：主循环派发的 `subagent` / `spawn_swarm` 成员，内部工具调用默认按 `review` 模型审核判定（快速审核模型未配置或判 HIGH 仍转人工；`git_commit` 恒人工），主 agent 切到 `yolo` 时子代理同步；手动启动的子代理仍随会话权限档。同一条消息里多个 subagent 并行跑（默认 2 个同时运行），在设置 → 上下文与运行调「子代理并行数」（`OWC_SUB_AGENT_CONCURRENCY`，1–16，默认 2；1 = 逐条串行；只限并发数，调用都会执行）；「spawn_swarm 并发成员数」（`OWC_SPAWN_SWARM_CONCURRENCY`，2–16，默认 4）决定一次 swarm 同时跑几个成员。两项都热生效
- **权限请求**：弹出权限卡片，三个选项：
...
- **子代理**（底部面板「子代理」标签页）：汇总本会话全部 `subagent`/`spawn_swarm`——按调用分组，swarm 显示完成/失败/运行中聚合计数与逐项实时进度，可内联展开转录；消息轨道里的子代理卡片也实时刷新（swarm 逐项状态、角色徽标、模型名、轮次、工具数，合成轮有「汇总」状态行），转录展示完整内部消息流（折叠到最近 20 条）；刷新页面后从历史恢复
- **时间线**（底部面板「时间线」标签页）：检查点列表 → diff 查看 → 「完整回滚」或「仅文件」回滚（二次确认），新建检查点；diff 视图在所有快照后端都提供完整 unified diff（逐文件 hunk 级「接受/恢复」，系统无 git 或快照目录不可见时降级为变更摘要）；顶部为可交互**会话树**（上限 50 个节点，非活动分支淡化），悬停任意节点可「继续」（检出到该节点，后续消息形成新分支）或「分叉」为新会话
...
- **模式切换**：会话空闲时可在头部直接切换沙盒模式、快照的「每轮自动 / 仅手动」、命令后端、Python 环境与 Node 环境；运行中会暂时禁用切换。本机会话（kind=local）沙盒固定 `关闭`，不能切换

## 模型与成本

- **模型服务商**：可保存并独立启用多个接口配置，接口类型三种——Anthropic Messages、OpenAI Chat Completions 与 OpenAI Responses（`POST /responses`，思维以 reasoning summary 流返回，历史思维链不回传）；每个服务商可自动拉取或手动添加自己的模型，同名模型互不覆盖。每个服务商可配**自定义请求体**（JSON，如 `{"temperature": 0.7, "max_tokens": 8192}`），浅合并进每次模型请求；`model`/`messages`/`stream`/`tools`/`system` 为保留字段不可覆盖。SSE 流的 idle 超时可在设置 → 模型目录 →「流空闲超时 (ms)」调整（`OWC_PROVIDER_STREAM_IDLE_MS`：0 = 关闭，留空用服务商内置默认），下次注册（重启或服务商配置变更）后生效
- **会话中热切换模型**：统一列表显示为 `模型ID【服务商】`，下轮生效；账本按新窗口重算，模态不兼容的历史内容替换为占位描述。**运行中也可以切换**（模型与思考档在输入区弹层随时可改，不打断在途请求，下一轮生效；提示条会写明「下一轮生效」）——若此刻正跑在备选模型上（限流自动降级），切换会立即放弃该降级、回到你新选的主模型；会话的 agent 模式（plan / code / goal）与沙盒、环境等仍须空闲时改
...
- 手机（≤768px）为单列布局：导航收进左上角面板图标触发的左侧滑出菜单（图标+文字竖向列表，视图与功能两分组）；菜单选视图后收缩为左侧纯图标栏、右侧整屏展示对应面板（再点当前视图图标或遮罩/Esc 收起），设置走同一导航轨整页打开（点项钻取、可返回）；底部面板变全屏 sheet（标签默认只显示 上下文/时间线/成本，其余收进可展开的第二行；面板内容全部断行/堆叠适配，无左右滚动）、编辑器/diff 变覆盖主区的全屏临时视图；核心操作（权限卡、结构化交互、队列、启停 run、切换会话）均可在 3 次点击内完成，按钮点击目标 ≥44px。chat 模式在手机端侧栏变覆盖式抽屉（遮罩/Esc 关闭、选中会话自动收起）。
- 平板竖屏（768–1024px，3:2 / 16:10）直接用桌面三栏布局，不走移动抽屉。
...
- **生效范围**：切换只影响本轮任务的后续 turn，不改会话模型字段；上下文窗口与能力按新模型重新解析（窗口变小由现有水位安全网兜底（默认 85%）），用量与成本按实际生效的 provider/model 逐 turn 记账。子代理不继承 fallback 链（子代理走角色模型链）
...
**离线模式**在 **设置 → 联网服务** 开启（`OWC_OFFLINE`，热生效）：关闭 server 自身的启动期/周期性出站——更新检查、远程模型目录/定价的后台同步、汇率在线刷新。不影响模型 API、联网搜索/抓取、MCP 与扩展联网，这些由会话与配置显式驱动的请求照常发出。

**出站 User-Agent**（设置 → 通用，`OWC_USER_AGENT`，热生效）：给全部出站 HTTP 请求换 UA（联网搜索/抓取、模型 API、MCP 等），留空用官方默认；更新检查与更新下载仍走官方 UA，env-sim 模拟开启时优先。
...
- 设置 → **服务信息** 展示 Server/Core 版本与协议版本；命令行 `owc --version` 打印服务版本。执行器、存储上限/数据目录、Python/Node 环境默认值、沙盒代理拦截域名、用量日志清理等系统级参数也在该页签调整。
- **用量日志清理**（设置 → 服务信息，默认关闭）：`usage-events.jsonl`（成本报表数据源）按所选模式定期清理（启动时 + 每小时一次，保存设置立即生效一次）：`已删除会话超时清理`（仅已删除会话的事件保留超过指定天数后清理，未删除保留）/ `统一超时清理`（所有事件超过指定天数清理，不分会话）/ `已删除立即、未删除超时`（已删除会话的事件立即清理，未删除超过指定天数清理）/ `已删除立即、未删除保留`（已删除会话的事件立即清理，未删除不清理）。相关环境变量：`OWC_USAGE_LOG_CLEANUP_MODE`、`OWC_USAGE_LOG_RETENTION_DAYS`（保留天数，默认 365）。
...
- `context-saver`（上下文节省，Context Saver）：默认启用，由 context-manager 扩展改名而来（version 0.3.0；持久化状态自动迁移，旧键配置原样继承）。负责上下文节省策略：滚动驱逐（自动/手动逐出工具结果，lag/interval/off 策略与默认节省/超级节省驱逐模式热调）、上下文条目管理（恢复/固定/再逐出/查看 artifact 原文）与选择性上下文（pin 消息或文件不逐出、排除路径不进入上下文组装/repo map/索引）。停用后 agent 循环不再自动驱逐、read_artifact 的驱逐联动与 pin/排除选择均不生效，上下文面板中驱逐策略/选择性上下文/上下文条目三段隐藏，相关 REST 端点返回 409。压缩（水位强制自动压缩 + `/compact` 手动压缩）、预算与 cache 断点是核心功能，不随扩展开关；自动压缩水位（默认 85%）可在设置「上下文」页签调整
...
同一次 `spawn_swarm` 的成员还拥有三个专属工具 `swarm_board_post` / `swarm_board_read` / `swarm_wait`，经共享讨论板互相协调：板文件为会话数据目录下 `subagents/swarm-<id>-board.jsonl`（append-only JSONL）。启动时板上写入 roster 系统贴（全员角色/模型/分工摘要互相可见），成员起止写生命周期帖。帖子支持结构化协作语义：`kind`（finding/question/request/blocker/progress/decision）、私聊（`to=<成员名>`，仅收发双方可见，板文件仍全量可审计）、引用回复（`replyTo=<帖 id>`）与 `priority=high`；读取支持 `since` 增量与 kind/from/to/mine 过滤，超过 80 条时旧段广播帖按成员+kind 确定性聚合（私聊与 @提及帖始终保留原文，不进聚合）。`swarm_wait` 让成员一次性等待他人回复（不占额外 LLM 轮次）：默认等发给自己的私聊或 @提及，`from=<成员>` 等指定成员（其已结束/失败且无新帖时提前返回），`any=true` 等任意新帖；默认 120 秒、上限 300 秒，等待期间占用并发槽。`spawn_swarm` 汇总时在聚合结论后附 Board digest（按 kind 分组统计 + 决议/阻塞帖置顶 + 最后几条）。传 `synthesize={role?, maxTurns?}`（默认关）可在全部成员结束后加一次无工具合成轮：以指定档位模型把各成员结论与板 digest 合并为结构化报告（关键发现/分歧/风险/决议/待办，多一次 LLM 调用成本；合成失败自动回落原始结论并标注错误）。消息轨道的 swarm 卡片逐项显示角色徽标（极致/平衡/快速/廉价四档着色）与实际生效模型名，合成轮有独立的「汇总」状态行。
