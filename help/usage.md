...
- **界面**：左侧可折叠会话栏（新建 / 重命名 / 删除 / 分享 / 分支，顶部搜索框按标题过滤），中间窄列消息流，底部输入框。支持 Markdown / LaTeX 渲染，粘贴或选择图片上传（小图直接内嵌，大图落盘为引用，单条消息最多 3 张）。上翻阅读时右下出现「回到底部」浮钮（与工作台同款）；首页空态建议每屏 3 条，「换一批」轮转。
