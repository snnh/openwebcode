...
设置 → **远程访问** 分区随时展示当前监听地址、回环/非回环状态与 token 认证说明；非回环监听期间持续展示风险提示。非回环 + token 只是准入门槛，仍建议只在受信网络使用。
...
界面为五区布局：**活动栏**（左侧窄条：会话/文件/SCM/设置等入口）、**侧栏**、**主区**（会话轨道与输入框，永远是中心）、**底部面板**（问题/输出/后台任务/沙盒等）、**状态栏**。布局状态（侧栏宽度、面板开合）按工作区保存在本机。
...
完整键位见设置 → **快捷键** 分区；0.4.0 暂不支持自定义键位。输入框聚焦时全局快捷键不抢键。
...
- **只读代码视图**：Shiki 高亮 + 行号 + 行列跳转的统一代码查看形态，从工具卡、问题面板、SCM diff 或 Quick Open 打开，`Esc`/关闭即回到对话；0.4.0 不含编辑器与 hunk 级接受/拒绝（属 0.5.0）。

## 移动端与 PWA

- 浏览器「安装到主屏」（PWA manifest，standalone 模式）后可像应用一样打开；不做离线缓存——应用强依赖实时连接。
- 移动端（≤768px）为单列布局：活动栏变顶部横条、侧栏变全屏抽屉、底部面板变全屏 sheet；核心操作（权限卡、结构化交互、队列、启停 run、切换会话）均可在 3 次点击内完成，按钮点击目标 ≥44px。
