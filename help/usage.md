...
界面为五区布局：**活动栏**（左侧窄条：会话/文件/SCM/设置等入口）、**侧栏**、**主区**（会话轨道与输入框，永远是中心）、**底部面板**（上下文/时间线/子代理/沙盒/成本/性能等标签页）、**状态栏**。桌面端状态栏信息已并入底部面板页签条右侧（状态、模式、模型、窗口占用、成本），移动端保留独立状态栏；打开会话时先显示骨架屏，避免欢迎页闪烁。布局状态（侧栏宽度、面板开合等）保存在本机浏览器。
...
- **只读代码视图**：Shiki 高亮 + 行号 + 行列跳转的统一代码查看形态，从工具卡、问题面板、SCM diff 或 Quick Open 打开，`Esc`/关闭即回到对话。工具卡文件变化、Problems 跳转、SCM diff 还可打开 Monaco 编辑器/分栏 diff（随需加载，未打开不付出体积），支持逐 hunk 接受/拒绝；加载失败或移动端降级为只读视图。
...
- **持久 shell**：agent 的 bash 工具默认复用每会话一个持久 shell（沙盒内 pty），`cd` 切换的目录、设置的环境变量跨调用保持；pty 不可用（如旧版 core）时透明回退一次性执行，不报错。`run_in_background` 后台任务仍走一次性 job；Windows 上 `pwsh` 后端在 AppContainer 沙盒下暂回退为一次性执行（cmd 不受影响）
- **桌面通知**：设置 → 通用开启（默认关，首次需浏览器授权通知权限）；页面失焦时，权限待批、结构化交互待答与 run 终态会弹系统通知，点击跳回对应会话

## 真终端

会话可打开真正的宿主机终端（xterm.js 随需加载，未打开不付出体积）：server 经 core 的 `pty.*` RPC 桥接（Windows ConPTY / Linux openpty），WebSocket 双向 JSON 帧直连。
...
- **上下文窗口**：会话头部实时显示窗口占用 `45k/128k · 38%`（≥70% 变黄、≥85% 变红）与缓存命中胶囊（悬停细分读取/写入/未缓存输入）；底部面板页签条（移动端为状态栏）同步显示 `窗口 N%`
- **上下文用量**（底部面板「上下文」标签页）：顶部「上下文窗口」区给出大号占用表、按段（消息/工具结果/repoMap/压缩摘要/系统/其他）堆叠的 token 归因条、本轮/累计缓存命中与水位提示；下方为 token/成本/预算明细，支持 lag/interval/off 策略热调、工具调用/概览压缩、条目逐出/回写/pin，以及 artifact 原文查看
...
- **模型服务商**：可保存并独立启用多个 Anthropic Messages / OpenAI Chat Completions 接口配置；每个服务商可自动拉取或手动添加自己的模型，同名模型互不覆盖。每个服务商可配**自定义请求体**（JSON，如 `{"temperature": 0.7, "max_tokens": 8192}`），浅合并进每次模型请求；`model`/`messages`/`stream`/`tools`/`system` 为保留字段不可覆盖
