...
- **压缩检查点行**：`/compact` 或达到自动压缩水位（默认 85%）强制压缩开始时，消息流尾部出现「正在压缩上下文」活动行；完成后原位沉降为常驻检查点行（模式徽标 + 手动/强制标注 + 被压缩条数与 token 估算），带摘要的记录可展开查看摘要与指令清单；压缩失败转为常驻错误行（可关闭）。多次压缩各自留行，刷新后从上下文账本逐条还原；行只是视图投影，历史消息原文不动
...
- **上下文窗口**：会话头部实时显示窗口占用 `45k/128k | 38%`（默认 ≥70% 变黄、≥85% 变红；红线跟随设置的自动压缩水位，黄线为水位 −15 个百分点）与缓存命中胶囊（悬停细分读取/写入/未缓存输入）；底部面板页签条同步显示 `窗口 N%`
