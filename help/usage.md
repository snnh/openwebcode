...
- **会话组织（分组 / 归档 / 多选批量）**：会话列表按**手工标签组**组织——会话项上的文件夹按钮可「移动到分组 / 移出分组 / 新建分组」，组头显示会话数并可折叠、重命名、删除（删除分组只把组内会话送回「未分组」，不删会话）；折叠状态记在本机浏览器。分组与归档状态存在服务端会话 meta（换浏览器/换设备一致）。**归档**＝把会话移出默认列表、进底部的「已归档」区（默认收起），归档会顺带释放它占用的常驻资源（索引内存与文件监听、repo map 扫描缓存；索引文件仍在磁盘，回来看时自动重建），但**不删历史、快照与 artifacts**，仍可继续使用（发送消息即自动撤销归档）。为避免归档中的会话还在改文件，**会话有活动就拒绝归档**：运行中的 run、仍在跑的后台任务、开着的终端都会返回「先停掉再归档」。列表顶部的「多选」按钮进入批量模式：勾选（或「全选」当前列表）后可批量删除（二次确认并列出前几个标题）、归档、置顶/取消置顶；批量归档会自动跳过仍在运行的会话并提示跳过数量。
- **资源占用会自己收敛**：长时间用下来不需要手动清理——前端按会话缓存的历史消息有上限（单会话最多保留约 3 页，超出的最旧部分丢弃，再往上翻会重新拉取；同时只保留最近用过的 10 个会话的分页缓存），服务端的符号索引与文件监听也按「空闲 30 分钟自动释放、最多同时保留 8 个项目」回收（索引文件仍在磁盘上，回去看时自动重建，不影响索引内容）。归档会话会立即释放它占用的常驻资源。
...
- **待回答区（权限卡 / 结构化提问 / 计划批准 / 运行队列共用）**：这四类「等你操作」的方块渲染在输入框上方（消息列表之外），并且**同时只展开一张**——默认展开队列里最早的那个（权限确认优先，因为它在卡住这轮运行），其余收成一行摘要（`待回答` / `待批准` 标记 + 主操作按钮，可直接回答，不必先展开）；点卡头三角收起/展开，收起状态按卡片记忆（本会话内切标签、刷新都不丢），新出现的卡默认展开。卡片内容超高时**卡内滚动**（上限约为可用高度的一半），消息列表始终保留约 3 行上下文、输入框始终完整可见；有新的待回答卡出现时消息列表会自动滚到底，方便看清问题来自哪段对话
- **权限请求**：弹出权限卡片，三个选项：
...
- **文件树**（侧栏「文件」视图）：懒加载；文件只读预览——超过 2000 行可点「加载更多」分页续读，图片（png/jpg/gif/webp/svg/ico/bmp）直接显示，Markdown 可切换「渲染/源码」双态，头部可一键「在编辑器中打开」。单目录有条目上限，超出时列表末尾提示「条目过多」并指引用 Quick Open / `@` 搜索（此前超出部分会被静默丢弃）；目录读取有 10s 客户端缓存，展开/折叠与切回视图不再每次重新拉取，agent 写文件后自动刷新
...
- **子代理**（底部面板「子代理」标签页）：汇总本会话全部 `subagent`/`spawn_swarm`——按调用分组，swarm 显示完成/失败/运行中聚合计数与逐项实时进度，可内联展开转录；顶部有状态筛选（全部/运行中/完成/失败），历史多时一眼筛出在跑或失败的那批；消息轨道里的子代理卡片也实时刷新（swarm 逐项状态、角色徽标、模型名、轮次、工具数，合成轮有「汇总」状态行），卡片展开后可直接「在标签中打开」跳去主区标签页；转录展示完整内部消息流（折叠到最近 20 条）；刷新页面后从历史恢复
- **子代理的主区标签页**：每个 `subagent`/`spawn_swarm` 调用一个标签，标签名是 `#序号 · 代理名 · 任务摘要`（序号＝本会话内该次派生的发生顺序，同名代理也能一眼区分；swarm 显示 `#n Swarm ×N 项`），固定按发生顺序排列。**只有存在运行中或未关闭的标签时这条标签栏才出现**（没有子代理时不再常驻占位；任务清单 chip 在时会保留这一行）；子代理完成后标签留在原位显示完成/失败点，需手动关闭；标签多到横向滚动时，当前标签会自动滚入视野，行尾还有「全部 ⋯」下拉列全并可直接跳转。点开标签页，**每个运行行与组头都会显示该子代理实际生效的模型**（四档角色徽标 + 模型短名，悬停看完整 id），已完成的条目默认显示**结论摘要 + 「复制结论」**，不必再逐项展开转录
...
- **只读代码视图**：Shiki 高亮 + 行号 + 行列跳转的统一代码查看形态，从工具卡、问题面板、SCM diff 或 Quick Open 打开，`Esc`/关闭即回到对话。工具卡文件变化、Problems 跳转、SCM diff 还可打开 Monaco 编辑器/分栏 diff（随需加载，**首屏不预下载**，未打开不占用包体积），支持逐 hunk 接受/拒绝；加载失败降级为只读视图，窄屏（≤1024px）则为覆盖主区的全屏临时视图。两点性能约定：**超大内容不高亮**——超过 60k 字符时直接渲染纯文本并给出说明（可复制或在编辑器中打开），因为 Shiki 在渲染线程同步执行（实测约 5ms/KB）；高亮结果在 transition 中提交，先显示可交互的纯文本再替换为高亮。悬停或聚焦「在编辑器中打开」会提前预热 Monaco chunk，把首次打开的首字节等待提前到点击之前。
