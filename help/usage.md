...
界面为五区布局：**活动栏**（左侧窄条：会话/文件/SCM/设置等入口）、**侧栏**、**主区**（会话轨道与输入框）、**底部面板**（上下文/时间线/子代理/沙盒/成本/性能等标签页）、**状态栏**。桌面端状态栏信息已并入底部面板页签条右侧（状态、模式、模型、窗口占用、成本），移动端保留独立状态栏；打开会话时先显示骨架屏，避免欢迎页闪烁。布局状态（侧栏宽度、面板开合等）保存在本机浏览器。
