...
// 背压阈值通过 buildServer 既有配置项 wsBackpressureLimits 压到 256KB，
// 与 server/test/event-stream-backpressure.test.ts 同款手法，不改被测代码。
...
try {
  ...
  const slowWs = new WebSocket(`${base}/api/events`);
  let slowCloseCode = 0;
  let slowClosedAt = 0;
  ...
  // 正常客户端
  const healthy = await connectWs(base);

  // 灌入事件流；每次发布让出事件循环，给正常客户端留出走空窗口（同集成测试）
  const floodStart = performance.now();
  const payload = "x".repeat(FLOOD_PAYLOAD_BYTES);
  for (let i = 0; i < FLOOD_EVENTS; i++) {
    events.publish({ source: "server", type: `flood-${i}`, payload });
    await new Promise((resolve) => setImmediate(resolve));
  }

  // 正常客户端应收齐全部 flood 事件——背压只踢慢客户端，不影响健康客户端
  await waitFor(() => healthy.received.length >= FLOOD_EVENTS, 20000);
  const healthyDrainMs = performance.now() - floodStart;
  ...
  await writeResult("slow-client", [
    ...
    params: { floodEvents: FLOOD_EVENTS, floodPayloadBytes: FLOOD_PAYLOAD_BYTES, maxBufferedBytes: 256 * 1024 },
  });
