import { spawn } from "node:child_process";
import { readFile } from "node:fs/promises";
import path from "node:path";
/**
 * Hooks：可信配置（与 yolo 同级，文档警示）。项目 <cwd>/.owc/hooks.json 覆盖/补充全局
 * <dataDir>/hooks.json——数组按项目在前合并；每次事件现读两份配置（热更新免重启），
 * 坏 JSON 告警跳过不阻断。
 *
 * 执行语义：shell 命令 stdin=JSON 事件负载，5s 超时杀进程。exit 0=放行；
 * exit 2=否决（仅 PreToolUse 有意义，stderr 回填 LLM）；其他非零/超时=告警不阻断（hook.failed 事件）。
 */
export class HookRunner {
    globalPath;
    events;
    static TIMEOUT_MS = 5000;
    static MAX_STDERR = 4000;
    constructor(globalPath, events) {
        this.globalPath = globalPath;
        this.events = events;
    }
    async run(event, payload) {
        const entries = await this.loadEntries(event, payload);
        for (const entry of entries) {
            if (!matchesMatcher(entry.matcher, payload.tool))
                continue;
            const result = await this.execute(entry.command, payload);
            if (result.exitCode === 0)
                continue;
            if (result.exitCode === 2 && event === "PreToolUse") {
                const reason = result.stderr.trim() || `Hook 拒绝了工具 ${payload.tool ?? event}`;
                return { blocked: true, reason };
            }
            // 其他非零与超时：告警不阻断
            this.events?.publish({
                source: "server",
                type: "hook.failed",
                sessionId: payload.sessionId,
                payload: {
                    event,
                    command: entry.command,
                    ...(result.exitCode === null ? { timeout: true } : { exitCode: result.exitCode }),
                    stderr: result.stderr.slice(0, HookRunner.MAX_STDERR),
                },
            });
        }
        return {};
    }
    /** 项目级在前、全局在后合并；坏 JSON 告警并按空配置处理 */
    async loadEntries(event, payload) {
        const project = await this.readHooksFile(path.join(payload.cwd, ".owc", "hooks.json"), payload.sessionId);
        const global = await this.readHooksFile(this.globalPath, payload.sessionId);
        return [...(project[event] ?? []), ...(global[event] ?? [])];
    }
    async readHooksFile(file, sessionId) {
        let raw;
        try {
            raw = await readFile(file, "utf8");
        }
        catch {
            return {}; // 不存在：正常路径
        }
        try {
            return normalizeHooksConfig(JSON.parse(raw));
        }
        catch (error) {
            this.events?.publish({
                source: "server",
                type: "hook.failed",
                sessionId,
                payload: { event: "config", command: file, stderr: `hooks.json 解析失败：${error instanceof Error ? error.message : String(error)}` },
            });
            return {};
        }
    }
    execute(command, payload) {
        return new Promise((resolve) => {
            const shell = process.platform === "win32" ? "cmd.exe" : "sh";
            const args = process.platform === "win32" ? ["/d", "/s", "/c", command] : ["-c", command];
            const stderr = [];
            let settled = false;
            const settle = (exitCode) => {
                if (settled)
                    return;
                settled = true;
                clearTimeout(timer);
                resolve({ exitCode, stderr: stderr.join("") });
            };
            let child;
            try {
                child = spawn(shell, args, { cwd: payload.cwd, windowsHide: true, windowsVerbatimArguments: true });
            }
            catch (error) {
                resolve({ exitCode: 1, stderr: error instanceof Error ? error.message : String(error) });
                return;
            }
            const timer = setTimeout(() => {
                child.kill("SIGKILL");
                stderr.push("(hook timeout)");
                settle(null);
            }, HookRunner.TIMEOUT_MS);
            child.stderr?.on("data", (chunk) => stderr.push(chunk.toString("utf8")));
            child.on("error", (error) => {
                stderr.push(error.message);
                settle(1);
            });
            child.on("close", (code) => settle(code));
            try {
                child.stdin?.write(JSON.stringify(payload));
                child.stdin?.end();
            }
            catch {
                // stdin 写入失败不影响命令执行与退出码判定
            }
        });
    }
}
/** matcher：精确工具名、"前缀*" 前缀匹配、"*" 全匹配；无 tool 的事件仅 "*" 命中 */
export function matchesMatcher(matcher, tool) {
    if (matcher === "*")
        return true;
    if (tool === undefined)
        return false;
    if (matcher.endsWith("*"))
        return tool.startsWith(matcher.slice(0, -1));
    return tool === matcher;
}
/** 校验并归一 hooks.json：事件名白名单、entry 需 matcher+command 字符串，坏条目丢弃 */
export function normalizeHooksConfig(value) {
    if (!value || typeof value !== "object")
        return {};
    const config = {};
    const events = ["PreToolUse", "PostToolUse", "UserPromptSubmit", "Stop", "SessionStart"];
    for (const event of events) {
        const entries = value[event];
        if (!Array.isArray(entries))
            continue;
        const valid = entries.filter((entry) => Boolean(entry) && typeof entry === "object" &&
            typeof entry.matcher === "string" && entry.matcher !== "" &&
            typeof entry.command === "string" && entry.command !== "");
        if (valid.length > 0)
            config[event] = valid;
    }
    return config;
}
