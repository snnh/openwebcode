...
import { EXTENSION_API_VERSION } from "./types.js";
...
export class ExtensionManager {
    ...
    list() {
        return this.manifests.map((manifest) => {
            ...
            const status = !state.enabled ? "disabled" : connected && !hostError ? "running" : "error";
    ...
    async install(sourcePath) {
        ...
        if (!(await stat(sourcePath).catch(() => undefined))?.isDirectory())
            throw new Error("Extension path must be a directory");
        const manifest = await readManifest(sourcePath);
        if (manifest.official)
            throw new Error("Third-party extension cannot claim official status");
        if (this.manifests.some((item) => item.id === manifest.id))
            throw new Error(`Extension ID already exists: ${manifest.id}`);
        ...
        if (!installed)
            throw new Error("Installed extension was not discovered");
        return installed;
    }
    async uninstall(id) {
        const manifest = this.manifests.find((item) => item.id === id);
        if (!manifest)
            throw new Error("Extension not found");
        if (manifest.official || !manifest.directory)
            throw new Error("Official extensions cannot be uninstalled");
        await rm(manifest.directory, { recursive: true, force: true });
        delete this.states[id];
        await this.saveStates();
        await this.restart();
    }
    async transformContext(payload) {
        const result = await this.hook("context.beforeBuild", payload);
        return { messages: result.messages, ...(result.metadata ? { metadata: result.metadata } : {}) };
    }
    async beforeSend(payload) {
        const result = await this.hook("message.beforeSend", payload);
        return { messages: result.messages, ...(result.metadata ? { metadata: result.metadata } : {}) };
    }
    async beforeTool(payload) {
        return this.hook("tool.beforeExecute", payload);
    }
    async hook(hook, payload) {
        try {
    ...
    async loadStates() {
        let stored = { version: 1, extensions: {} };
        try {
    ...
    async discoverThirdParty() {
        ...
        for (const entry of await readdir(this.root, { withFileTypes: true }).catch(() => [])) {
            if (!entry.isDirectory() || !entry.name.startsWith("owc-ext-"))
                continue;
            const directory = path.join(this.root, entry.name);
            try {
                const manifest = await readManifest(directory);
                if (ids.has(manifest.id))
                    throw new Error(`Duplicate or reserved extension ID: ${manifest.id}`);
                ids.add(manifest.id);
    ...
    async startHost() {
        ...
        const initialized = await this.request("initialize", { states: this.states, manifests: this.manifests });
    ...
    request(method, params) {
        const child = this.child;
        if (!child?.connected)
            return Promise.reject(new Error("Extension Host is not connected"));
        const id = randomUUID();
        return new Promise((resolve, reject) => {
            ...
            child.send({ id, method, ...(params ? { params } : {}) });
        });
    }
}
async function readManifest(directory) {
    const raw = JSON.parse(await readFile(path.join(directory, "manifest.json"), "utf8"));
    if (!raw.id || !/^[a-z0-9][a-z0-9-]{1,63}$/.test(raw.id))
        throw new Error("manifest.id is invalid");
    if (!raw.name || !raw.version || !raw.description)
        throw new Error("manifest requires name, version and description");
    if (raw.apiVersion !== EXTENSION_API_VERSION)
        throw new Error(`Unsupported apiVersion ${raw.apiVersion ?? "missing"}`);
    ...
    if (!Array.isArray(raw.permissions) || raw.permissions.some((permission) => typeof permission !== "string" || !allowedPermissions.has(permission)))
        throw new Error("manifest.permissions contains an unsupported permission");
    const entry = raw.entry ?? "index.js";
    if (path.isAbsolute(entry) || entry.split(/[\\/]/).includes(".."))
        throw new Error("manifest.entry must stay inside the extension directory");
    return {
        id: raw.id,
        name: raw.name,
        version: raw.version,
