...
export class ContentLensService {
    sessions;
    provider2;
    constructor(sessions, provider2) {
        this.sessions = sessions;
        this.provider2 = provider2;
    }
    async translate(sessionId, messageId, targetLanguage, glossary = {}) {
        if (!this.provider2.configured)
            throw new Error("provider2 未配置，内容透镜无法翻译");
        ...
        if (!source.trim())
            throw new Error("Message has no translatable text");
        ...
        await writeFile(file, `${JSON.stringify({ key, text: completion.text, createdAt: new Date().toISOString() }, null, 2)}\n`, "utf8");
        return { text: completion.text, cached: false };
    }
    async explain(sessionId, text, targetLanguage) {
        if (!this.provider2.configured)
            throw new Error("provider2 未配置，内容透镜无法解析");
        if (!(await this.sessions.get(sessionId)))
            throw new Error("Session not found");
        const selected = text.trim();
        if (!selected || selected.length > 200)
            throw new Error("选中文本长度必须为 1–200 字符");
        const completion = await this.provider2.complete({
