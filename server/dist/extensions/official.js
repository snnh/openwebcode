...
export function optimizeAttention(payload, config) {
    ...
    const candidates = payload.messages
        ...
        if (/必须|不要|禁止|始终|要求|约束|目标|must|never|always|require/i.test(text))
            score += 40;
        if (/错误|失败|异常|error|failed|exception/i.test(text))
            score += 20;
