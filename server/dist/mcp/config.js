...
export const isHttpConfig = (config) => "url" in config;
function validate(name, value) {
    if (!value || typeof value !== "object")
        return undefined;
    const record = value;
    if (typeof record.url === "string" && record.url !== "") {
        return { url: record.url, ...(record.headers && typeof record.headers === "object" ? { headers: record.headers } : {}) };
    }
    if (typeof record.command === "string" && record.command !== "") {
        return {
            command: record.command,
            ...(Array.isArray(record.args) ? { args: record.args.map(String) } : {}),
            ...(record.env && typeof record.env === "object" ? { env: record.env } : {}),
            ...(typeof record.cwd === "string" ? { cwd: record.cwd } : {}),
        };
    }
    return undefined;
}
async function readConfigFile(filePath, result) {
    let parsed;
    try {
 ...
 */
export async function loadMcpConfig(dataDir, cwd) {
    const result = { servers: {}, skipped: [] };
    await readConfigFile(path.join(dataDir, "mcp.json"), result);
    if (cwd)
        await readConfigFile(path.join(cwd, ".owc", "mcp.json"), result);
    return result;
}
