...
class Client {
    ...
    closed = false;
    ready;
    constructor(serverName, config, timeoutMs) {
    ...
    async start() {
        if (!isHttpConfig(this.config)) {
            ...
            this.child.on("exit", (code) => {
                if (!this.closed)
                    this.failAll(new Error(`MCP server ${this.serverName} 已退出（code ${code}）${this.stderrTail ? `：${this.stderrTail.trim()}` : ""}`));
            });
        }
        await this.request("initialize", {
            protocolVersion: PROTOCOL_VERSION,
            capabilities: {},
            clientInfo: { name: "openwebcode", version: "0.1.0" },
        });
    ...
    handleMessage(message) {
        if (typeof message.id === "number" && ("result" in message || "error" in message)) {
            ...
            if (rpcError)
                entry.reject(new Error(`MCP ${this.serverName} 错误 ${rpcError.code ?? ""}: ${rpcError.message ?? "unknown"}`));
            else
    ...
    sendRpc(message) {
        if (this.closed)
            throw new Error(`MCP server ${this.serverName} 连接已关闭`);
        if (isHttpConfig(this.config))
            void this.postHttp(message);
        else {
            if (!this.child)
    ...
    async postHttp(message) {
        ...
        try {
            ...
            if (message.id === undefined)
                return; // 通知：202/空响应属正常（会话 id 已在上方捕获）
            if (!response.ok)
...
export async function connectMcpServer(name, config, options = {}) {
