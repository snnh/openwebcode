...
export class ZfsBackend {
    ...
    name = "zfs";
    metadataPath;
    constructor(sessionRoot, workspace, dataset, runner) {
    ...
    async diff(id) {
        ...
        if (result.code !== 0)
            throw new Error(`zfs diff failed (${result.code})`);
    ...
    async must(operation, args) {
        ...
        if (result.code !== 0)
            throw new Error(`zfs ${operation} failed (${result.code})`);
    }
}
