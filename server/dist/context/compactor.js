...
function mergeInstructions(previous, extracted) {
...
function renderBlock(block) {
    ...
    if (block.type === "thinking")
        return `[思考] ${block.text.slice(0, 200)}`;
    if (block.type === "tool_call")
        return `[调用工具 ${block.name}] ${JSON.stringify(block.input).slice(0, 300)}`;
    if (block.type === "tool_result")
        return `[工具结果${block.isError ? "（错误）" : ""}] ${block.content.slice(0, 800)}`;
    return "";
}
/** 渲染待压缩区段为纯文本转录，超长时保头留尾。 */
export function renderSpan(span, budget = 30_000) {
...
function ruleBasedToolcalls(span) {
    const lines = [];
    for (const message of span) {
        for (const block of message.content) {
            if (block.type === "tool_call")
                lines.push(`- [工具] ${block.name}(${JSON.stringify(block.input).slice(0, 80)})`);
            else if (block.type === "tool_result")
                lines.push(`  → ${block.isError ? "错误" : "完成"}：${block.content.slice(0, 120).replace(/\s+/g, " ")}`);
            else if (block.type === "text" && message.role === "user")
                lines.push(`- [用户] ${block.text.slice(0, 120).replace(/\s+/g, " ")}`);
        }
    }
...
export class Compactor {
    ...
    deps;
    keepTail;
    constructor(sessions, provider2, deps = {}, keepTail = 10) {
        this.sessions = sessions;
        this.provider2 = provider2;
        this.deps = deps;
        this.keepTail = keepTail;
    }
    async compact(sessionId, mode, options = {}) {
        ...
        if (this.provider2.configured) {
            ...
            if (mode === "overview")
                instructions = mergeInstructions(instructions, extractInstructions(summary));
    ...
    recordProvider2Usage(sessionId, usage) {
        if (!this.deps.usageLog)
            return;
