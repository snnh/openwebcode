export class ProviderError extends Error {
    ...
    retryable;
    retryAfterMs;
    constructor(kind, message, retryable, retryAfterMs, options) {
...
export function classifyHttpError(status, message, retryAfterMs) {
    if (status === 401)
        return new ProviderError("authentication", message, false);
    if (status === 403)
        return new ProviderError("permission", message, false);
    if (status === 404)
        return new ProviderError("not_found", message, false);
    if (status === 400 || status === 409 || status === 422)
        return new ProviderError("invalid_request", message, false);
    if (status === 408 || status === 429)
        return new ProviderError("rate_limit", message, true, retryAfterMs);
    if (status === 529)
        return new ProviderError("overloaded", message, true, retryAfterMs);
    if (status >= 500)
        return new ProviderError("overloaded", message, true, retryAfterMs);
    return new ProviderError("unknown", message, false);
}
export function normalizeProviderError(error, streamStarted = false) {
