...
/** 最小 pty fake：openPty 立即成功，inputPty 回送 sentinel 行（init 退出码可配）。 */
function makePtyCore(initCode: number) {
  let ptySeq = 0, openCalls = 0;
  const emitters = new Map<number, EventEmitter>();
  const core = {
    on() { return core; },
    async configureSession() { return { sandboxCapability: "advisory" as const }; },
    async openPty() { openCalls += 1; const ptyId = ++ptySeq; emitters.set(ptyId, new EventEmitter()); return { ptyId, sandboxCapability: "advisory" } as PtyOpenResult; },
    async inputPty({ ptyId, data }: { ptyId: number; data: string }) {
      const rand = /__OWC_DONE_([0-9a-f]{12})_/.exec(Buffer.from(data, "base64").toString("utf8"))?.[1];
      const emitter = emitters.get(ptyId);
      if (emitter && rand) setTimeout(() => emitter.emit("output", { data: Buffer.from(`\r\n__OWC_DONE_${rand}_${initCode}__\r\n`, "utf8").toString("base64") }), 0);
...
describe("持久 shell 初始化失败缓存回收", () => {
  it("disposeSession 清理 initFailed，环境修复后重新尝试建壳", async () => {
    ...
    // 建壳失败后记入失败缓存：同一 session:backend 直接回退一次性 exec，不再付开壳代价
    ...
    // 释放其它会话不影响本会话的失败缓存：仍然直接回退，不重新建壳
