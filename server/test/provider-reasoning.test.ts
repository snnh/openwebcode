...
describe("provider reasoning parameters", () => {
  ...
  it("sends OpenAI reasoning_effort only when enabled", async () => {
    const bodies: Array<Record<string, unknown>> = [];
    const fetch = sseFetch(bodies);
    await drain(new OpenAICompatibleProvider({ baseURL: "https://example.invalid/v1", fetch }).streamChat(request({ effort: "high" })));
    await drain(new OpenAICompatibleProvider({ baseURL: "https://example.invalid/v1", reasoningEffort: false, fetch }).streamChat(request({ effort: "high" })));
  ...
  it("omits an empty tools field instead of advertising an unavailable tool schema", async () => {
    ...
    const fetch = sseFetch(openAiBodies);
    await drain(new OpenAICompatibleProvider({ baseURL: "https://example.invalid/v1", fetch }).streamChat(request()));
