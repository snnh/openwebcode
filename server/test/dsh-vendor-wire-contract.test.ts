 ...
 *
 * 这里刻意不自己实现一套宽松校验：`assertSessionWireEvent` / `surfaceOpOf` / follow 首帧的
 * `assistantStream` 硬要求都是**从 vendor 产物原样求值**得到（见 test/helpers/dsh-vendor-wire.ts），
 * 参数形状直接读 vendor 生成的 typert 描述符。vendor 缺失时整组跳过。
 */
...
import {
  assertOpenPayloadShape,
  vendorDescriptors,
  vendorWireAssertions,
  VENDOR_READY,
} from "./helpers/dsh-vendor-wire.js";
...
function projectionDeps(messages: ChatMessage[] = history()): DshWireDeps["projection"] {
  ...
  return {
    ...
    agent: {
      ...
      isRunning: () => false,
      abort: () => true,
      enqueueSteering: async () => ({ id: "x", position: 1, reused: false }),
      enqueueFollowUp: async () => ({ id: "y", position: 1, reused: false }),
      listQueue: async () => [],
      updateQueue: async () => undefined,
      removeQueue: async () => false,
    },
...
describe.skipIf(!VENDOR_READY)("dsh 线格式（vendor 真实校验）", () => {
  it("D1：投影出的每条事件通过 vendor assertSessionWireEvent，surface-eligible 事件带 surfaceOp=append", () => {
    ...
    for (const record of records) {
      if (surfaceTypes.has(record.event.type)) {
        expect(record.event.surfaceOp, record.event.type).toBe("append");
        expect(assertions?.surfaceOpOf(record.event as never), record.event.type).toBe("append");
      } else {
        // 非面事件带 surfaceOp 会被 vendor 反向拒绝（不许乱标）
        expect(record.event.surfaceOp, record.event.type).toBeUndefined();
        expect(assertions?.surfaceOpOf(record.event as never), record.event.type).toBeUndefined();
      }
    }
    // 反向守卫：漏掉 surfaceOp 时 vendor 校验确实会炸（证明这条断言不是自欺）
    const userEvent = records.find((record) => record.event.type === "user/message")?.event;
    expect(userEvent).toBeDefined();
    const { surfaceOp: _dropped, ...withoutMarker } = userEvent as { surfaceOp?: unknown };
    expect(() => assertions?.assertSessionWireEvent(withoutMarker)).toThrow(/surface-eligible and requires a surfaceOp/);
  });

  it("D1：follow/page 的 records 元素（外层 {type:'event',event}）也逐条通过 vendor 校验", async () => {
    const assertions = vendorWireAssertions();
    const follow = await projectSessionFollowSnapshot({ ...projectionDeps() } as never, { request: { address: { kind: "session", sessionId: "s1" }, maxMessages: 50 } });
    const snapshot = ("value" in follow ? follow.value : {}) as { records: Array<{ event: unknown }> };
    for (const record of snapshot.records) expect(() => assertions?.assertSessionWireEvent(record.event)).not.toThrow();
    const page = await projectSessionPage({ ...projectionDeps() } as never, { request: { address: { kind: "session", sessionId: "s1" }, throughSeq: 99, maxMessages: 2 } });
    const pageValue = ("value" in page ? page.value : {}) as { records: Array<{ event: unknown }> };
    expect(pageValue.records.length).toBeGreaterThan(0);
    for (const record of pageValue.records) expect(() => assertions?.assertSessionWireEvent(record.event)).not.toThrow();
  });

  it("D2：follow 首帧带 assistantStream 基线，通过 vendor 首帧校验与 strict codec", async () => {
    const assertions = vendorWireAssertions();
    const descriptors = await vendorDescriptors();
    const projected = await projectSessionFollowSnapshot({ ...projectionDeps() } as never, {
      request: { address: { kind: "session", sessionId: "s1" }, maxMessages: 50, assistantStream: true },
    });
    ...
    const descriptor = descriptors.get("session/follow");
  ...
  it("D3：session/control 描述符无参数，我方端点接受客户端真实 payload 并产出基线（strict codec 通过）", async () => {
    const descriptors = await vendorDescriptors();
    ...
    // 客户端编码形态：无参数端点也是 {args:{}}；多带参数会被形状校验挡下
    assertOpenPayloadShape(descriptor!, { args: {} });
    ...
    const sink: DshMuxChannel = { send: (frame) => frames.push(frame), close: () => {} };
  ...
  it("D9：asOfSeq 与记录 seq 同口径（列表/控制基线 = 记录条数 = 快照 cursor）", async () => {
    ...
    expect(sessionRecordsCount(messages)).toBe(full.records.length);
    expect(sessionRecordsCount([])).toBe(0);
    expect(sessionRecordsCount([message("a1", "assistant", [{ type: "text", text: "x" }])])).toBe(3);
