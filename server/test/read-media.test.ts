...
describe("sniffMedia：魔数权威 + 扩展名仅视频兜底", () => {
  it("sniffMedia 全矩阵：魔数命中、avif/未知、扩展名兜底且不覆盖魔数", () => {
    const cases: Array<[Uint8Array, string]> = [
      ...
      [withHeader(ascii("GIF89a")), "image/gif"],
      [withHeader([...ascii("RIFF"), 0, 0, 0, 0, ...ascii("WEBP")]), "image/webp"],
      [withHeader([0, 0, 0, 32, ...ascii("ftyp"), ...ascii("isom")]), "video/mp4"],
      [withHeader([0, 0, 0, 32, ...ascii("ftyp"), ...ascii("3gp4")]), "video/3gpp"],
      [withHeader([0, 0, 0, 32, ...ascii("ftyp"), ...ascii("qt  ")]), "video/quicktime"],
      [withHeader([0x1a, 0x45, 0xdf, 0xa3]), "video/webm"],
      [withHeader([...ascii("RIFF"), 0, 0, 0, 0, ...ascii("AVI ")]), "video/x-msvideo"],
      [withHeader(ascii("FLV")), "video/x-flv"],
      [withHeader([0x00, 0x00, 0x01, 0xba, 0x00]), "video/mpeg"],
    ];
    for (const [bytes, expected] of cases) {
      expect(sniffMedia(bytes)?.mediaType).toBe(expected);
    }
    ...
    expect(sniffMedia(withHeader([0xff, 0xd8, 0xff, 0xe0]), "clip.mpg")?.mediaType).toBe("image/jpeg"); // 魔数优先
  });
});
...
describe("collectToolMedia / provider 消息形状", () => {
  ...
  it("openai 系合成 user 消息：兼容端 image_url/video_url data URL；Responses 端 input_image，视频降级占位文本", () => {
    const media = [{ kind: "image" as const, mediaType: "image/png", data: PNG_DATA }, { kind: "video" as const, mediaType: "video/mp4", data: MP4_DATA }];
    expect(openaiCompatibleMediaMessage(media)).toEqual({
      role: "user",
      content: [
    ...
    expect(openaiResponsesMediaMessage(media)).toEqual({
      role: "user",
      content: [
...
describe("fetchMedia", () => {
  ...
  it("成功：content-type + 魔数一致 → image（重定向终点扩展名兜底不影响）", async () => {
  ...
  it("拒绝：非媒体 content-type / 魔数不符 / 私网 URL / HTTP 错误", async () => {
    await expect(fetchMedia("https://cdn.example.com/page", { fetchImpl: binaryFetch(pngBytes, "text/html"), lookupImpl: publicLookup })).rejects.toThrow(/Use web_fetch/);
    await expect(fetchMedia("https://cdn.example.com/photo", { fetchImpl: binaryFetch(new TextEncoder().encode("plain text"), "image/png"), lookupImpl: publicLookup })).rejects.toThrow(/magic bytes/);
