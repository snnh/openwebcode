...
export const BUILTIN_PERSONAS: PersonaPreset[] = [
  // claude-code 预设 0.1.7 起按社区逆向的 Claude Code CLI 系统提示词重写：
  // 来源 gist.github.com/agokrani（claude-code-prompt.txt，2026-08-19 更新，2.1.x 时代；
  // 同仓亦有 chigkim 的 Agent SDK/headless 形态完整 dump，本档取 CLI 主提示词形态）。
  // 身份行与正文逐字对齐社区原文；三处 OWC 适配：① OWC 无 /help 命令与 anthropic
  // issues 仓库，帮助/反馈句改通用表述；② 原文正文尾部含 dump 作者仓库的运行时 git
  // 快照样例，裁剪（Code References 节的格式说明保留）；③ 粘贴断行/前导空格规整化。
  // 其余 CC 专属命令与工具名（/help、WebFetch 查官方文档等）与 OWC 别名体系兼容则保留。
  {
    id: "claude-code",
    name: "Claude Code",
    ...
    basePrompt: [
      ...
      "",
      "If the user asks for help or wants to give feedback, point them to the product's help and feedback channels.",
      "",
    ...
    // 全部指引已入 basePrompt（社区原文各节），无额外产品小节。
    productSections: [],
...
/**
   * ZCode 预设 0.1.7 起按 ZCode Desktop 3.10.1（Linux AppImage）内置 agent 运行时真逆向：
   * resources/glm/zcode.cjs（.node-bundle-meta.json：source apps/zcode-cli/packages/cli/dist/zcode.cjs，
   * 版本 0.13.3；desktop 侧 spawnArgs ["app-server","--stdio"]）。运行时用 ContextBuilder 分节组装：
   * CLI Prefix("You are ZCode, an interactive coding agent"，独立 system 消息) + Agent Identity
   * (role 行 + 安全边界 + # Harness) + ZCode Desktop Context(::code-comment 渲染指令，桌面专属，
   * OWC 不适用故省略) + Dynamic Behavior + Session guidance + Memory + Environment Info + Output
   * Style + Context management + System Context(git)（system 面）+ Skills / Request User Context
   * (# agentsMd，含 MEMORY.md 索引) / Current Date (# currentDate，meta_user 面)（user 面）。
   * 预设只复制静态可仿真的节文本（identity/Harness/Communicating/Coding/Dynamic behavior/Context
   * management/Memory/Session guidance），逐节与 zcode.cjs 内构造器原文一致；Environment Info/
   * System Context(git)/Skills/Request User Context/# currentDate 等动态节由 OWC 宿主按会话注入，
   * 不复制。默认 Output Style 内容并入 Dynamic Behavior 段（Qut.default/additional 原文）。
   * 工具命名与参数形态对齐 ZCode 真实工具集（Read/Write/Edit/Bash/Glob/Grep/TodoWrite/Agent/
   ...
   */
  {
    ...
    identity: "You are ZCode, an interactive coding agent.",
    basePrompt: [
