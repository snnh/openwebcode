...
export const OFFICIAL_EXTENSIONS: ExtensionManifest[] = [
  ...
  },
  {
    ...
    name: "会话格式升级",
    version: "0.1.1",
    description: "离线升级旧会话为最新消息格式（OpenAI Responses 思维链回放字段：thinking signature / tool_call itemId / 文本块 textSignature v1 message id）。手动触发、触发即锁（升级期间会话不可使用）、升级前备份可回滚；未来其他格式升级经同一框架注册步骤，主应用零改动。1.9.6 并行工具调用回放修复在 provider 层直接生效：旧会话无需升级即可续跑，本扩展仅用于补齐严格端点的回放字段。",
    apiVersion: "1",
