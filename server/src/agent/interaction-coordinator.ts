...
export class InteractionCoordinator {
  ...
  private async mutate<T>(sessionId: string, change: (items: InteractionRequest[]) => T): Promise<T> {
    const previous = this.writes.get(sessionId) ?? Promise.resolve(); const operation = previous.catch(() => undefined).then(async () => { const items = await this.read(sessionId); const result = change(items); await writeUtf8Atomically(this.file(sessionId), `${JSON.stringify({ version: 1, items } satisfies Document, null, 2)}\n`); return result; }); this.writes.set(sessionId, operation); try { return await operation; } finally { if (this.writes.get(sessionId) === operation) this.writes.delete(sessionId); }
  }
