...
export interface HookPayload {
  sessionId: string;
  cwd: string;
  /** 工具事件携带工具名；UserPromptSubmit/Stop/SessionStart 无 tool（matcher 仅 "*" 命中） */
  tool?: string;
  input?: unknown;
  /** PostToolUse：结果摘要（截断） */
  result?: unknown;
  /** UserPromptSubmit：展开后的用户消息正文（截断） */
  prompt?: string;
}
...
export function normalizeHooksConfig(value: unknown): HooksConfig {
  ...
  const events: HookEvent[] = ["PreToolUse", "PostToolUse", "UserPromptSubmit", "Stop", "SessionStart"];
