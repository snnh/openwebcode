...
import { TotpAuthService, TOTP_TICKET_TTL_MS, isLoopbackOrLAN, timingSafeHashEqual } from "./auth-totp.js";
...
import type { BindLinkSpec, FallbackModelEntry, MessageContent, NodeEnv, PermissionMode, PythonEnv, SandboxMode, SandboxNetwork, SessionMeta, ShellBackend, SnapshotMode, TextContent } from "./sessions/types.js";
import type { ChatAssistantStore, ChatConfigService, ChatPythonEnv, ChatRunner, ChatSessionStore } from "./chat/index.js";
import type { ChatAssistant, ChatConfig, ChatImageInput, ChatSessionMeta, ChatShare } from "./chat/index.js";
...
const CHAT_IMAGE_BODY_LIMIT = 20 * 1024 * 1024;
...
function isSharePublicRoute(pathname: string): boolean {
...
export async function buildServer(dependencies: ServerDependencies): Promise<FastifyInstance> {
  ...
  let chatLanUnauthCache: boolean | undefined;
  const chatLanUnauthenticated = async (): Promise<boolean> => {
    if (chatLanUnauthCache !== undefined) return chatLanUnauthCache;
    const config = dependencies.chatConfig ? await dependencies.chatConfig.get() : {};
    chatLanUnauthCache = config.lanUnauthenticated !== false;
    return chatLanUnauthCache;
  };
  ...
  app.put<{ Body: ChatConfig }>("/api/chat/config", async (request, reply) => {
    ...
    chatLanUnauthCache = request.body.lanUnauthenticated !== false;
    return chatConfig.get();
  });
