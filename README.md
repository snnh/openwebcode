...
4. 输入框描述任务，回车发送。若选择「托管工作区」，源目录会先复制到镜像盘；需要回写时，在底部「文件」面板点「同步回源」，先核对差异再确认。
...
**快照回滚**：每轮用户消息前自动检查点；后端自动探测 Btrfs/ZFS/ReFS，兜底 git 影子仓库；可选「托管工作区」（项目活在 VHDX/qcow2 镜像盘上，差分链快照毫秒级、可分支）。托管工作区不会在关闭或删除会话时自动覆盖源目录；可随时在「文件」面板生成三方差异，确认后只回写无冲突的改动。
