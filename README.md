...
开发机实测（Windows x86-64 1.6.5版本，5000 条消息基准数据集；基准脚本与验收门禁在 [`scripts/bench/`](./scripts/bench/)）：

| 组件 | 内存占用 | CPU（折合为单核的95%时间占用） | 关键指标 |
|---|---|---|---|
| server（Node 服务层） | 空闲约 86 MiB；载入 5000 消息大会话后稳态约 100 MiB | 0.8% | 大会话冷载 23ms、历史分页 p50 0.49ms；上下文增量构建 p50 0.33ms（较全量构建 33× 加速）；agent 主循环每轮堆增量 0.9 MiB；事件分发 5300+ events/s；10 万文件符号索引查询 p50 约 16–21ms |
| core（C 执行器） | 空闲约 9 MiB；重负载扫描峰值约 25 MiB，结束即回落 | 低于0.5% | 3.4MB 文件读取 8ms；全仓索引扫描（数十万文件）25s 内完成且内存可控 |
| 浏览器端 | 5000 消息会话满载堆约 92 MiB | - | 长列表滚动 p50 59.9 fps；输入回显 p50 27ms；持续滚动内存增长 0.1%（无泄漏）；聊天/工作台/分享页按需分包，首屏脚本 475 KB |

生产环境参考（v1.6.5，Debian 13 x86-64，systemd 常驻实测）：server 114 MiB + 扩展宿主 50 MiB + core 1.9 MiB（server 较 v1.5.0 的 135 MiB 下降约 15%），CPU 95%时间占用低于0.5%。
