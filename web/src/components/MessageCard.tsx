...
import { ToolCallListGroup, groupCallsFromBlocks } from "./ToolCallListGroup";
  ...
  return (
    <article className={`message ${message.role}${turn !== undefined ? ` turn-${turn % 2 === 0 ? "even" : "odd"}` : ""}`} ref={articleRef} data-message-id={message.id}>
      ...
      {groupContentBlocks(content).map((group, index) => group.kind === "tool-group"
        ? <ToolCallListGroup key={index} calls={groupCallsFromBlocks(group.blocks, toolResults, running)} onOpenDiff={onOpenDiff} />
        : <ContentBlock key={index} block={group.block} sessionId={sessionId} liveSubagents={liveSubagents} toolResults={toolResults} running={running} onOpenDiff={onOpenDiff} />)}
