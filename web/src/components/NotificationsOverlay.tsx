...
export function NotificationsOverlay({ open, notifications, onActivate, onDismiss, onClearAll, onClose }: {
  ...
  return (
    <Overlay open={open} label={t("通知中心", "Notifications")} className="notifications-overlay" onClose={onClose}>
      ...
      {notifications.length === 0 ? (
        <p className="muted-empty panel-empty">{t("暂无通知。后台任务完成、诊断与源代码管理更新会出现在这里。", "No notifications. Background task completions, diagnostics, and source control updates appear here.")}</p>
      ) : (
