...
import type { ModelInterfaceType, ModelProviderProfileView, WebCapability, WebProviderProfileView, WebProviderType } from "../../lib/contracts";
    ...
    <>
      ...
      <div className="server-settings-group">
        ...
        <div className="catalog-edit-form">
          ...
          <div className="catalog-form">
            {(webForm.provider === "custom" && webForm.capabilities.includes("search")) && <input value={webForm.searchBaseURL} placeholder={t("Search Base URL", "Search Base URL")} onChange={(event) => setWebForm((current) => ({ ...current, searchBaseURL: event.target.value }))} spellCheck={false} />}
            {(webForm.provider === "custom" && webForm.capabilities.includes("fetch")) && <input value={webForm.fetchBaseURL} placeholder={t("Fetch Base URL（含 {url}）", "Fetch Base URL (contains {url})")} onChange={(event) => setWebForm((current) => ({ ...current, fetchBaseURL: event.target.value }))} spellCheck={false} />}
          </div>
