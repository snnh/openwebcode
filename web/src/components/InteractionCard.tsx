...
export function InteractionCard({ item, onRespond }: { item: InteractionRequest; onRespond(answer: unknown): void }): ReactElement {
  ...
  return <section className="interaction-card" aria-label={item.title}>
    ...
    <div className="interaction-actions"><button className="btn small" onClick={submit} disabled={(item.kind === "text" && !text.trim()) || ((item.kind === "single_select" || item.kind === "multi_select") && selected.length === 0)}>{item.kind === "confirm" ? t("确认", "Confirm") : t("提交回答", "Submit answer")}</button>{item.kind === "confirm" && <button className="btn small" onClick={() => onRespond(false)}>{t("取消", "Cancel")}</button>}</div>
  </section>;
}
