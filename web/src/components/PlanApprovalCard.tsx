...
export function PlanApprovalCard({ item, onRespond }: { item: InteractionRequest; onRespond(answer: PlanApprovalAnswer): void }): ReactElement {
  const { t } = useI18n();
  const [mode, setMode] = useState<"view" | "edit" | "reject">("view");
  const [draft, setDraft] = useState(item.prompt);
  const [feedback, setFeedback] = useState("");
  return <section className="interaction-card plan-approval-card" aria-label={item.title}>
    ...
    <div>
      ...
      {mode === "edit" && <>
        <button className="btn small" onClick={() => onRespond({ decision: "edit", plan: draft })} disabled={!draft.trim()}>{t("提交修改并批准", "Submit edits and approve")}</button>
        <button className="btn small" onClick={() => { setDraft(item.prompt); setMode("view"); }}>{t("取消", "Cancel")}</button>
      </>}
      {mode === "reject" && <>
        <button className="btn small" onClick={() => onRespond({ decision: "reject", feedback: feedback.trim() })}>{t("确认拒绝", "Confirm rejection")}</button>
