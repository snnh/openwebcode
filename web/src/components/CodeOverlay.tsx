...
export function CodeOverlay({ sessionId, path, onEdit, onClose }: {
  ...
  return (
    <Overlay open label={path} className="code-overlay" onClose={onClose}>
      <header className="wb-overlay-header">
        ...
        <button className="icon-btn" aria-label={t("关闭（Esc）", "Close (Esc)")} onClick={onClose}><Icon name="x" /></button>
      </header>
