...
export function installAppFetchMock(options: AppFetchMockOptions = {}): void {
  ...
  const handler = vi.fn(async (input: RequestInfo | URL): Promise<Response> => {
    ...
    if (url.endsWith("/api/sandbox/capabilities")) return json({ appcontainer: true, jobobject: true, off: true, wsb: { available: false, reason: "测试环境" }, bindLink: { available: false, reason: "测试环境" } });
