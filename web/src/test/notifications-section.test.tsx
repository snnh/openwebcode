...
function renderSection(notifications: AppNotification[]) {
  ...
  const view = render(
    <NotificationsSection notifications={notifications} onActivate={onActivate} onDismiss={onDismiss} onClearAll={onClearAll} onMarkAllRead={onMarkAllRead} />,
  );
  return { view, onActivate, onDismiss, onClearAll, onMarkAllRead };
}

describe("通知中心（设置页签）", () => {
  it("列出通知、未读高亮、逐条清除与全部清除", () => {
    const { view, onDismiss, onClearAll } = renderSection([item(), item({ id: "n2", kind: "error", text: "诊断更新：2 项失败", read: true })]);
  ...
  it("挂载时触发全部标记已读（角标清零）", () => {
    const { onMarkAllRead } = renderSection([item()]);
    expect(onMarkAllRead).toHaveBeenCalledTimes(1);
  });

  it("点击条目触发跳转回调", () => {
    const target = { sessionId: "s1", view: "problems" as const };
    const { view, onActivate } = renderSection([item({ target })]);
