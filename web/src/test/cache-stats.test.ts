...
describe("cacheHitRate", () => {
  it("命中率口径 cacheRead / (inputTokens + cacheRead)：含全未缓存（0）与纯缓存命中（1）两端", () => {
    ...
    expect(cacheHitRate({ inputTokens: 5_000, outputTokens: 100, cacheRead: 0, cacheWrite: 0 }).rate).toBe(0);
    expect(cacheHitRate({ inputTokens: 0, outputTokens: 0, cacheRead: 30_000, cacheWrite: 0 }).rate).toBe(1);
  });

  it("空数据归 null：全零、null/undefined 输入", () => {
    const zero = cacheHitRate({ inputTokens: 0, outputTokens: 0, cacheRead: 0, cacheWrite: 0 });
