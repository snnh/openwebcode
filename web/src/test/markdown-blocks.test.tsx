...
describe("splitMarkdownBlocks", () => {
  ...
  it("代码围栏：内部空行不拆、~~~ 与长闭合、异种围栏为内容、未闭合归尾块", () => {
    // 代码围栏内的空行不拆分
    const text = "before\n\n```js\nconst a = 1;\n\nconst b = 2;\n```\n\nafter";
...
describe("Markdown 分块增量渲染", () => {
  it("稳定块不重复解析：增长只动末块、相同文本重渲染零重算", async () => {
    // 流式增长只重渲染末尾块，稳定块不重复解析
    renderCounts.clear();
    ...
    // 父组件用相同文本重渲染时任何块都不重复解析
    renderCounts.clear();
    const text = "para1\n\npara2";
    const { rerender: rerenderSame } = render(<Markdown>{text}</Markdown>);
    await waitFor(() => expect(renderCounts.get("para2")).toBeGreaterThanOrEqual(1));
    const snapshot = new Map(renderCounts);

    rerenderSame(<Markdown>{text}</Markdown>);
    rerenderSame(<Markdown>{text}</Markdown>);
