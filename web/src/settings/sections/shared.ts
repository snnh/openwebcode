...
export const SETTINGS_FIELD_EN: Record<string, { label: string; description?: string }> = {
  ...
  dshCompatEnabled: { label: "Enable dsh compatibility mode", description: "Off by default; when enabled, the dsh SPA is served on a separate port with dsh plugins loaded (host and client plugins are trusted code, ~yolo, outside the sandbox and permission chain); zero resident state while off (port not listening)" },
  dshPort: { label: "dsh compatibility mode port", description: "Dedicated listening port for dsh compatibility mode (1024-65535, default 3211)" },
  dshUiPath: { label: "Custom dsh UI directory", description: "Empty = built-in vendor (server/assets/dsh-web from scripts/fetch-dsh-web.mjs); a directory overrides manifest.json + static/ + plugins/" },
