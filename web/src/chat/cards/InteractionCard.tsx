...
export function InteractionCard({ item, onRespond }: InteractionCardProps): ReactElement {
  ...
  const submit = (): void => onRespond(
    item.kind === "confirm" ? true
      : item.kind === "text" ? text
        : item.kind === "single_select" ? selected[0]
          : selected,
  );
  return (
    <section className="interaction-card" aria-label={item.title}>
      ...
      <div className="interaction-actions">
        <button
          className="btn small"
          onClick={submit}
          disabled={(item.kind === "text" && !text.trim()) || ((item.kind === "single_select" || item.kind === "multi_select") && selected.length === 0)}
        >
