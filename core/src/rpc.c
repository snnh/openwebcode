...
static int session_exec_policy(const char *id,const char *cwd,int *enabled,int *allow_network);
...
static int handle_exec_run(owc_rpc *rpc,const owc_json *id,const owc_json *params) {
    ...
    owc_exec_request request; owc_exec_result result; output_context context; int sandbox_enabled,allow_network;
    ...
    if(!session_exec_policy(session_id,cwd,&sandbox_enabled,&allow_network))return reply_error(rpc,id,-32002,"session cwd is not configured");
    request.command=command; request.cwd=cwd; request.session_id=session_id;request.sandbox_enabled=sandbox_enabled;request.allow_network=allow_network;
...
typedef struct { char *session_id; char *cwd; char *deny_paths[16]; size_t deny_count; int sandbox_enabled; int allow_network; } session_config;
...
static int session_exec_policy(const char *id,const char *cwd,int *enabled,int *allow_network){session_config *session=session_find(id);if(!session||!policy_path_equal(session->cwd,cwd))return 0;*enabled=session->sandbox_enabled;*allow_network=session->allow_network;return 1;}
