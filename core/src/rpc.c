...
static int handle_exec_run(owc_rpc *rpc,const owc_json *id,const owc_json *params) {
    ...
    if(!owc_exec_run(&request,&result)) { char message[96];(void)snprintf(message,sizeof(message),"failed to start or monitor command (system error %lu)",result.system_error);return reply_error(rpc,id,-32000,message); }
