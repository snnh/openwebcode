...
static int handle_fs_watch_poll(owc_rpc *rpc,const owc_json *id,const owc_json *p){
    ...
    fprintf(stderr,"[rpc-poll] raw count=%zu overflow=%d\n",events.count,events.overflow);
    ...
    fprintf(stderr,"[rpc-poll] after policy count=%zu\n",events.count);
