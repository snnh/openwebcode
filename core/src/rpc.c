...
/* Append a printf-formatted fragment to a heap JSON string, growing the buffer
 * to exactly the size needed first. Replaces the hand-rolled
 * "measure with snprintf(NULL,0) -> realloc -> write" idiom that every
 * incremental reply builder used to repeat, and centralizes the negative
 * vsnprintf return check (a negative length must not be cast to size_t).
 * Returns 1 on success; on failure the buffer is left valid and untouched and
 * the caller keeps ownership (each caller has its own cleanup set to free). */
static int json_appendf(char **buffer, const char *format, ...) {
    va_list args, copy; int length; size_t used; char *grown;
    va_start(args,format); va_copy(copy,args); length=vsnprintf(NULL,0,format,args); va_end(args);
    if(length<0) { va_end(copy); return 0; }
    used=strlen(*buffer);
    grown=(char *)realloc(*buffer,used+(size_t)length+1);
    if(!grown) { va_end(copy); return 0; }
    *buffer=grown;
    (void)vsnprintf(*buffer+used,(size_t)length+1,format,copy); va_end(copy);
    return 1;
}
...
static int handle_exec_run(owc_rpc *rpc,const owc_json *id,const owc_json *params) {
    ...
    if(!owc_exec_run(&request,&result)) { char message[384];if(result.shell_unavailable){(void)snprintf(message,sizeof(message),"%s executable was not found",shell_backend_name(request.shell_backend));return reply_error(rpc,id,-32000,message);}if(result.sandbox_reason[0])(void)snprintf(message,sizeof(message),"failed to start or monitor command (system error %lu): %s",result.system_error,result.sandbox_reason);else (void)snprintf(message,sizeof(message),"failed to start or monitor command (system error %lu)",result.system_error);return reply_error(rpc,id,-32000,message); }
...
typedef struct { char *session_id; char *cwd; char *deny_paths[16]; size_t deny_count; char *read_roots[16]; size_t read_root_count; char *write_roots[16]; size_t write_root_count; char *allow_paths[32]; size_t allow_count; int sandbox_enabled; int allow_network; int sandbox_mode; unsigned long job_memory_mb; unsigned long job_max_processes; char *bind_virt[16]; char *bind_backing[16]; int bind_read_only[16]; size_t bind_count; int network_filtered; char proxy_addr[144]; char *read_only_paths[32]; size_t read_only_count; } session_config;
...
static int configure_session(const char *id,const char *cwd,const owc_json *sandbox,char *err,size_t err_size,int *code){
    ...
    if(candidate.network_filtered){
        owc_sandbox_options grant_options;const char *grant_writes[48];size_t grant_write_count=0,g;char grant_reason[192];
        ...
        /* allowPaths are configured write roots too: with per-command grants
           ...
           grant_writes is bounded by writeRoots (16) + allowPaths (32). */
        ...
        grant_options.deny_paths=(const char *const *)candidate.deny_paths;grant_options.deny_count=candidate.deny_count;
...
#ifndef _WIN32
/* The backend a POSIX session will actually run under.  The default backend
 * is bubblewrap only: there is no silent Landlock fallback (Landlock cannot
 * express denyPaths, so falling back would quietly weaken isolation) - the
 * exec/pty children fail closed with guidance when the bwrap probe is not
 * enforced.  An explicit landlock mode keeps Landlock as a compatibility
 * backend, but its reported capability is capped at partial: denyPaths stay
 * unenforced for commands and its network rules are TCP-only.  The probes
 * are one-shot cached, so this is cheap. */
static void session_posix_capability(const session_config *session,owc_sandbox_result *probe){
    if(session->sandbox_mode!=(int)OWC_SANDBOX_MODE_LANDLOCK){owc_bwrap_probe(probe);return;}
    owc_landlock_probe(session->allow_network,probe);
    if(probe->status==OWC_SANDBOX_ENFORCED){char base[192];(void)snprintf(base,sizeof(base),"%s",probe->reason);probe->status=OWC_SANDBOX_PARTIAL;(void)snprintf(probe->reason,sizeof(probe->reason),"%.80s; denyPaths are not enforced for commands under Landlock (needs bubblewrap); network rules are TCP-only",base);}
}
#endif
...
static int handle_fs_stat_many(owc_rpc *rpc,const owc_json *id,const owc_json *p){
    ...
    for(i=0;i<paths->value.children.count;i++){
        ...
        if(!json_appendf(&result,"%s{\"path\":%s,\"type\":\"%s\",\"size\":%llu,\"modifiedMs\":%lld}",i?",":"",escaped,type_name(stat.type),stat.size,stat.modified_ms)){free(escaped);free(result);return reply_error(rpc,id,-32000,"out of memory");}
        free(escaped);
    }
    if(!json_appendf(&result,"]}")){free(result);return reply_error(rpc,id,-32000,"out of memory");}
    i=reply_result(rpc,id,result);free(result);return (int)i;
}
...
static int handle_fs_scan(owc_rpc *rpc,const owc_json *id,const owc_json *p){
    ...
    for(i=cursor;i<end;i++){char *escaped=owc_json_escape_string(scan.items[i].path);if(!escaped){free(result);scan_free(&scan);return reply_error(rpc,id,-32000,"out of memory");}if(!json_appendf(&result,"%s{\"path\":%s,\"type\":\"%s\",\"size\":%llu}",i==cursor?"":",",escaped,type_name(scan.items[i].type),scan.items[i].size)){free(escaped);free(result);scan_free(&scan);return reply_error(rpc,id,-32000,"out of memory");}free(escaped);}
    /* The closing suffix is appended through json_appendf so the buffer grows
     * first. An empty page (empty directory, or a cursor at the end of the
     * collection) skips the entry loop, so the buffer is still the 13-byte
     * header allocation and writing the suffix in place would run past it. */
    {const char *truncated=(scan.budget_truncated||scan.depth_truncated||scan.list_truncated)?"true":"false";
    int ok=end<scan.count?json_appendf(&result,"],\"nextCursor\":%zu,\"truncated\":%s}",end,truncated):json_appendf(&result,"],\"truncated\":%s}",truncated);
    if(!ok){free(result);scan_free(&scan);return reply_error(rpc,id,-32000,"out of memory");}}
...
static int handle_fs_watch_poll(owc_rpc *rpc,const owc_json *id,const owc_json *p){
    ...
    result=(char*)malloc(64);if(!result){owc_fs_watch_result_free(&events);return reply_error(rpc,id,-32000,"out of memory");}strcpy(result,"{\"events\":[");for(i=0;i<events.count;i++){char *path=owc_json_escape_string(events.events[i].path);if(!path){free(result);owc_fs_watch_result_free(&events);return reply_error(rpc,id,-32000,"out of memory");}if(!json_appendf(&result,"%s{\"path\":%s,\"kind\":\"%s\"}",i?",":"",path,events.events[i].kind)){free(path);free(result);owc_fs_watch_result_free(&events);return reply_error(rpc,id,-32000,"out of memory");}free(path);}if(!json_appendf(&result,"],\"overflow\":%s}",events.overflow?"true":"false")){free(result);owc_fs_watch_result_free(&events);return reply_error(rpc,id,-32000,"out of memory");}owc_fs_watch_result_free(&events);i=reply_result(rpc,id,result);free(result);return (int)i;
}
...
static int handle_job_start(owc_rpc *rpc,const owc_json *id,const owc_json *p){
    ...
    LeaveCriticalSection(&jobs_mutex);
    {char *escaped=owc_json_escape_string(job_id);char *result;int needed,ok;if(!escaped)return reply_error(rpc,id,-32000,"out of memory");needed=snprintf(NULL,0,"{\"jobId\":%s,\"state\":\"running\"}",escaped);result=needed<0?NULL:(char*)malloc((size_t)needed+1);if(result)(void)snprintf(result,(size_t)needed+1,"{\"jobId\":%s,\"state\":\"running\"}",escaped);free(escaped);if(!result)return reply_error(rpc,id,-32000,"out of memory");ok=reply_result(rpc,id,result);free(result);return ok;}
}
static int handle_job_cancel(owc_rpc *rpc,const owc_json *id,const owc_json *p){static const char *keys[]={"sessionId","jobId"};const char *session,*job_id;owc_job *job;char *escaped,*result;int needed,ok;if(!allowed_keys(p,keys,2))return reply_error(rpc,id,-32602,"job.cancel contains unknown fields");session=owc_json_get_string(owc_json_object_get(p,"sessionId"));job_id=owc_json_get_string(owc_json_object_get(p,"jobId"));if(!session||!job_id)return reply_error(rpc,id,-32602,"job.cancel requires sessionId and jobId");jobs_init();EnterCriticalSection(&jobs_mutex);job=find_job(session,job_id);if(!job){LeaveCriticalSection(&jobs_mutex);return reply_error(rpc,id,-32003,"job not found");}if(job->state==OWC_JOB_RUNNING)job->cancel=1;escaped=owc_json_escape_string(job->id);if(!escaped){LeaveCriticalSection(&jobs_mutex);return reply_error(rpc,id,-32000,"out of memory");}needed=snprintf(NULL,0,"{\"jobId\":%s,\"accepted\":true}",escaped);result=needed<0?NULL:(char*)malloc((size_t)needed+1);if(result)(void)snprintf(result,(size_t)needed+1,"{\"jobId\":%s,\"accepted\":true}",escaped);free(escaped);if(!result){LeaveCriticalSection(&jobs_mutex);return reply_error(rpc,id,-32000,"out of memory");}LeaveCriticalSection(&jobs_mutex);ok=reply_result(rpc,id,result);free(result);return ok;}
...
static int handle_pty_open(owc_rpc *rpc,const owc_json *id,const owc_json *p){
    ...
    /* sandbox=1 means "sandbox per the session policy": when the policy has
     * enabled=false (sandboxMode=off) the pty must fall back to unsandboxed,
     * otherwise the pty channel bypasses the disable semantics that
     * exec.run/job.start both obey. */
    ...
    if(!owc_pty_open(&options,pty_output_callback,pty_exit_callback,record,&handle,&open_result,&system_error)){
        char message[384];
        if(open_result.sandbox_reason[0])(void)snprintf(message,sizeof(message),"failed to open pty (system error %lu): %s",system_error,open_result.sandbox_reason);
        else (void)snprintf(message,sizeof(message),"failed to open pty (system error %lu)",system_error);
