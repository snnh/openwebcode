...
static owc_fs_error parent_fd(const char *root,const char *path,int *parent,char **leaf){int fd,nfd;const char *p=path,*slash;size_t n;char *part;
 ...
 if(path[0]=='/'){fd=open("/",O_RDONLY|O_DIRECTORY|O_CLOEXEC);p=path+1;}else{fd=open(root,O_RDONLY|O_DIRECTORY|O_CLOEXEC|O_NOFOLLOW);if(p[0]=='.'&&p[1]=='/')p+=2;}if(fd<0)return err();
...
static void fill(const struct stat *s,owc_fs_stat_result *r){r->type=S_ISREG(s->st_mode)?OWC_FS_TYPE_FILE:S_ISDIR(s->st_mode)?OWC_FS_TYPE_DIRECTORY:OWC_FS_TYPE_OTHER;r->size=(unsigned long long)s->st_size;r->modified_ms=(long long)s->st_mtim.tv_sec*1000+s->st_mtim.tv_nsec/1000000;}
...
/* Linux inotify is deliberately non-recursive: recursive watches need one
 * descriptor per child directory and would need a separate descriptor budget.
 * The capability reply advertises this limitation to callers. */
struct owc_fs_watch { int notify,directory,descriptor; };
static const char *watch_kind(uint32_t mask){return mask&IN_CREATE?"created":mask&IN_DELETE?"deleted":mask&(IN_MOVED_FROM|IN_MOVED_TO)?"renamed":"changed";}
