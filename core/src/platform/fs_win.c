...
/* ReadDirectoryChangesW with an overlapped completion event: a pending
 * directory read accumulates FILE_NOTIFY_INFORMATION records into the fixed
 * buffer until poll drains them.  Polling the completion event with a zero
 * timeout keeps the JSON-RPC reader responsive.  Each poll copies the batch
 * and re-arms immediately, so the change-loss window between the completion
 * and the next ReadDirectoryChangesW shrinks to a single memcpy (the protocol
 * documents that boundary; callers treat events as a staleness signal). */
#define OWC_FS_WATCH_BUFFER 65536u
struct owc_fs_watch { HANDLE directory; HANDLE event; OVERLAPPED overlapped; unsigned char buffer[OWC_FS_WATCH_BUFFER]; DWORD bytes; int recursive; int armed; int overflow_pending; int dead; };
...
static void watch_arm(owc_fs_watch *watch){
    ResetEvent(watch->event);
    if(!ReadDirectoryChangesW(watch->directory,watch->buffer,(DWORD)sizeof(watch->buffer),watch->recursive,watch_filters,&watch->bytes,&watch->overlapped,NULL)){watch->armed=0;if(GetLastError()==ERROR_NOTIFY_ENUM_DIR)watch->overflow_pending=1;else watch->dead=1;return;}
    watch->armed=1;
}
static const char *watch_kind(DWORD action){switch(action){case FILE_ACTION_ADDED:return "created";case FILE_ACTION_REMOVED:return "deleted";case FILE_ACTION_MODIFIED:return "changed";case FILE_ACTION_RENAMED_OLD_NAME:case FILE_ACTION_RENAMED_NEW_NAME:return "renamed";default:return "changed";}}
owc_fs_error owc_fs_platform_watch_open(const char *root,const char *path,int recursive,owc_fs_watch **result){owc_fs_watch *watch;wchar_t *directory;HANDLE handle;owc_fs_error error;if(!result)return OWC_FS_INVALID_ARGUMENT;*result=NULL;error=checked_open(root,path,0,OPEN_EXISTING,&handle,&directory);if(error)return error;CloseHandle(handle);/* Re-open the canonical directory without FILE_FLAG_OPEN_REPARSE_POINT so
     ReadDirectoryChangesW follows into the real directory; checked_open already
     ruled out attacker-controlled reparse points and verified the root bound. */handle=CreateFileW(directory,FILE_LIST_DIRECTORY,FILE_SHARE_READ|FILE_SHARE_WRITE|FILE_SHARE_DELETE,NULL,OPEN_EXISTING,FILE_FLAG_BACKUP_SEMANTICS|FILE_FLAG_OVERLAPPED,NULL);free(directory);if(handle==INVALID_HANDLE_VALUE)return winerr();watch=(owc_fs_watch*)calloc(1,sizeof(*watch));if(!watch){CloseHandle(handle);return OWC_FS_NO_MEMORY;}watch->directory=handle;watch->event=CreateEventW(NULL,TRUE,FALSE,NULL);if(!watch->event){CloseHandle(handle);free(watch);return OWC_FS_NO_MEMORY;}watch->overlapped.hEvent=watch->event;watch->recursive=recursive;watch_arm(watch);*result=watch;return OWC_FS_OK;}
owc_fs_error owc_fs_platform_watch_poll(owc_fs_watch *watch,size_t maximum_events,owc_fs_watch_result *result){unsigned char copy[OWC_FS_WATCH_BUFFER];DWORD got,offset;int overflow=0;if(!watch||!result||!maximum_events)return OWC_FS_INVALID_ARGUMENT;if(watch->dead)return OWC_FS_IO_ERROR;if(!watch->armed){watch_arm(watch);if(!watch->armed){if(watch->overflow_pending){watch->overflow_pending=0;result->overflow=1;}return OWC_FS_OK;}}if(WaitForSingleObject(watch->event,0)!=WAIT_OBJECT_0)return OWC_FS_OK;fprintf(stderr,"[poll] signaled\n");if(!GetOverlappedResult(watch->directory,&watch->overlapped,&got,FALSE)){DWORD error=GetLastError();watch->armed=0;if(error==ERROR_NOTIFY_ENUM_DIR){watch->overflow_pending=1;return OWC_FS_OK;}if(error==ERROR_OPERATION_ABORTED){watch->dead=1;return OWC_FS_IO_ERROR;}return OWC_FS_IO_ERROR;}fprintf(stderr,"[poll] got=%lu\n",(unsigned long)got);watch->armed=0;memcpy(copy,watch->buffer,sizeof(copy));watch_arm(watch);for(offset=0;offset<got;){FILE_NOTIFY_INFORMATION *record;size_t take;int n;wchar_t *name;char *path;owc_fs_watch_event *grown;if((size_t)(got-offset)<sizeof(*record)){overflow=1;break;}record=(FILE_NOTIFY_INFORMATION*)(copy+offset);fprintf(stderr,"[poll] rec offset=%lu next=%lu action=%lu nameLen=%lu\n",(unsigned long)offset,(unsigned long)record->NextEntryOffset,(unsigned long)record->Action,(unsigned long)record->FileNameLength);if(record->NextEntryOffset&&(record->NextEntryOffset<(DWORD)sizeof(*record)||record->NextEntryOffset>(got-offset))){overflow=1;break;}take=record->FileNameLength/sizeof(WCHAR);if(take>0&&result->count<maximum_events){name=(wchar_t*)malloc(((size_t)take+1)*sizeof(*name));if(!name)return OWC_FS_NO_MEMORY;memcpy(name,record->FileName,(size_t)take*sizeof(*name));name[take]=0;n=WideCharToMultiByte(CP_UTF8,WC_ERR_INVALID_CHARS,name,(int)take,NULL,0,NULL,NULL);fprintf(stderr,"[poll] take=%lu n=%d\n",(unsigned long)take,n);if(n<=0){free(name);overflow=1;}else{path=(char*)malloc((size_t)n+1);if(!path){free(name);return OWC_FS_NO_MEMORY;}if(!WideCharToMultiByte(CP_UTF8,WC_ERR_INVALID_CHARS,name,(int)take,path,n,NULL,NULL)){free(name);free(path);overflow=1;}else{path[n]=0;free(name);for(n=0;path[n];n++)if(path[n]=='\\')path[n]='/';fprintf(stderr,"[poll] path='%s'\n",path);grown=(owc_fs_watch_event*)realloc(result->events,(result->count+1)*sizeof(*grown));if(!grown){free(path);return OWC_FS_NO_MEMORY;}result->events=grown;result->events[result->count].path=path;result->events[result->count].kind=watch_kind(record->Action);result->count++;}}}else if(take>0)overflow=1;if(!record->NextEntryOffset){offset=got;break;}offset+=record->NextEntryOffset;}if(offset<got)overflow=1;result->overflow=overflow;return OWC_FS_OK;}
void owc_fs_platform_watch_close(owc_fs_watch *watch){if(!watch)return;if(watch->armed)CancelIoEx(watch->directory,&watch->overlapped);if(watch->event)CloseHandle(watch->event);if(watch->directory&&watch->directory!=INVALID_HANDLE_VALUE)CloseHandle(watch->directory);free(watch);}
#endif
