...
/* Keep the kernel notification pending and expose it through a non-blocking
 * poll.  The RPC layer can therefore cancel a watch without competing with a
 * long-running request or writing from a background thread. */
struct owc_fs_watch { HANDLE directory,event; OVERLAPPED overlap; unsigned char buffer[16384]; int recursive; };
static int watch_arm(owc_fs_watch *watch){DWORD ignored;ResetEvent(watch->event);memset(&watch->overlap,0,sizeof(watch->overlap));watch->overlap.hEvent=watch->event;return ReadDirectoryChangesW(watch->directory,watch->buffer,(DWORD)sizeof(watch->buffer),watch->recursive,FILE_NOTIFY_CHANGE_FILE_NAME|FILE_NOTIFY_CHANGE_DIR_NAME|FILE_NOTIFY_CHANGE_LAST_WRITE|FILE_NOTIFY_CHANGE_SIZE,&ignored,&watch->overlap,NULL)!=0;}
static const char *watch_kind(DWORD action){return action==FILE_ACTION_ADDED?"created":action==FILE_ACTION_REMOVED?"deleted":action==FILE_ACTION_RENAMED_OLD_NAME?"renamed":action==FILE_ACTION_RENAMED_NEW_NAME?"renamed":"changed";}
owc_fs_error owc_fs_platform_watch_open(const char *root,const char *path,int recursive,owc_fs_watch **result){owc_fs_watch *watch;owc_fs_error error;if(!result)return OWC_FS_INVALID_ARGUMENT;*result=NULL;watch=(owc_fs_watch*)calloc(1,sizeof(*watch));if(!watch)return OWC_FS_NO_MEMORY;error=checked_open_mode(root,path,FILE_LIST_DIRECTORY,OPEN_EXISTING,FILE_FLAG_OVERLAPPED,&watch->directory,NULL);if(error){free(watch);return error;}watch->recursive=recursive;watch->event=CreateEventW(NULL,TRUE,FALSE,NULL);if(!watch->event||!watch_arm(watch)){if(watch->event)CloseHandle(watch->event);CloseHandle(watch->directory);free(watch);return OWC_FS_IO_ERROR;}*result=watch;return OWC_FS_OK;}
owc_fs_error owc_fs_platform_watch_poll(owc_fs_watch *watch,size_t maximum_events,owc_fs_watch_result *result){DWORD bytes=0,error;size_t offset=0;if(!watch||!result)return OWC_FS_INVALID_ARGUMENT;if(!GetOverlappedResult(watch->directory,&watch->overlap,&bytes,FALSE)){error=GetLastError();if(error==ERROR_IO_INCOMPLETE)return OWC_FS_OK;if(error==ERROR_NOTIFY_ENUM_DIR){result->overflow=1;if(!watch_arm(watch))return OWC_FS_IO_ERROR;return OWC_FS_OK;}return OWC_FS_IO_ERROR;}if(!bytes)result->overflow=1;while(offset<bytes&&result->count<maximum_events){FILE_NOTIFY_INFORMATION *info=(FILE_NOTIFY_INFORMATION*)(watch->buffer+offset);size_t available=bytes-offset,names=(size_t)info->FileNameLength/sizeof(WCHAR);char *path;owc_fs_watch_event *grown;if(available<offsetof(FILE_NOTIFY_INFORMATION,FileName)||info->FileNameLength>available-offsetof(FILE_NOTIFY_INFORMATION,FileName)||info->FileNameLength%sizeof(WCHAR)){owc_fs_watch_result_free(result);return OWC_FS_IO_ERROR;}path=(char*)malloc((names+1)*4);if(!path){owc_fs_watch_result_free(result);return OWC_FS_NO_MEMORY;}if(!WideCharToMultiByte(CP_UTF8,WC_ERR_INVALID_CHARS,info->FileName,(int)names,path,(int)((names+1)*4),NULL,NULL)){free(path);owc_fs_watch_result_free(result);return OWC_FS_INVALID_UTF8;}path[WideCharToMultiByte(CP_UTF8,WC_ERR_INVALID_CHARS,info->FileName,(int)names,path,(int)((names+1)*4),NULL,NULL)]=0;grown=(owc_fs_watch_event*)realloc(result->events,(result->count+1)*sizeof(*grown));if(!grown){free(path);owc_fs_watch_result_free(result);return OWC_FS_NO_MEMORY;}result->events=grown;result->events[result->count].path=path;result->events[result->count].kind=watch_kind(info->Action);result->count++;if(!info->NextEntryOffset)break;if(info->NextEntryOffset>available){owc_fs_watch_result_free(result);return OWC_FS_IO_ERROR;}offset+=info->NextEntryOffset;}if(offset<bytes)result->overflow=1;if(!watch_arm(watch)){owc_fs_watch_result_free(result);return OWC_FS_IO_ERROR;}return OWC_FS_OK;}
void owc_fs_platform_watch_close(owc_fs_watch *watch){if(!watch)return;CancelIoEx(watch->directory,&watch->overlap);if(watch->event)CloseHandle(watch->event);if(watch->directory&&watch->directory!=INVALID_HANDLE_VALUE)CloseHandle(watch->directory);free(watch);}
#endif
