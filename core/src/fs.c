...
/* Keep in sync with OWC_SEARCH_TEXT_LIMIT in rpc.c (grep job): a matched
 * line's text is capped so one huge line cannot overflow the 32 MiB frame. */
#define OWC_FS_GREP_TEXT_LIMIT 512u
/* glob_walk / grep_walk recurse per directory level; cap the depth so an
 * extremely deep tree cannot overflow the stack.  Deeper directories are
 * skipped and reported through the existing truncated flag. */
#define OWC_FS_WALK_DEPTH_LIMIT 256u
...
/* Byte-oriented glob match ('*' = any run of bytes, '?' = exactly one byte).
 * Iterative on purpose: the previous version recursed once per character and
 * re-entered the matcher for every candidate suffix at each '*', so a pattern
 * like "*a*a*a*a*a*a*a*a*a*b" cost exponential time and pinned the single RPC
 * dispatch thread (fs.glob, fs.grep, index.scan and the search job all call
 * this).  The greedy loop keeps at most one backtrack point (the last '*') and
 * is O(len(pattern) * len(value)); both inputs are bounded by the RPC layer
 * (pattern <= 512 bytes, paths <= 4096 bytes).  Shared with the index.scan job
 * so include/exclude rules match fs.glob semantics exactly instead of growing
 * a second glob dialect. */
int owc_fs_match_pattern(const char *pattern,const char *value){
    const char *p=pattern,*v=value,*star=NULL,*resume=NULL;
    while(*v){
        if(*p=='*'){star=p++;resume=v;continue;}
        if(*p=='?'||*p==*v){p++;v++;continue;}
        if(star){p=star+1;v=++resume;continue;}
        return 0;
    }
    while(*p=='*')p++;
    return *p=='\0';
