...
/* Shared with the index.scan job so include/exclude rules match fs.glob
 * semantics exactly instead of growing a second glob dialect. */
int owc_fs_match_pattern(const char *pattern,const char *value){return wildcard(pattern,value);}
